    ...
    for (const ev of events) {
      ...
      for (const item of list) {
        ...
        ins.run({
          ...
          sku_id: skuId,
          site_name: siteName,
          goods_id: item.goodsId || item.goods_id || null,
          skc_id: item.skcId || item.skc_id || item.productSkcId || null,
          check_code: firstCheck.checkCode != null ? String(firstCheck.checkCode) : null,
          exception_reason: firstCheck.checkDesc || item.checkDesc || null,
          exception_time: exceptionTime,
          sku_spec: item.skuSpecString || item.specString || item.spec || null,
          raw_json: JSON.stringify(item),
          source: SETTLEMENT_ROBOT_SOURCE,
          source_received_at: Number(ev.received_at) || null,
          now,
  ...
  try {
    ...
    rows = db.prepare(`
      SELECT e.mall_id, e.sku_id, e.site_name, e.goods_id, e.skc_id,
        ...
        LEFT JOIN erp_mall_dictionary m ON e.mall_id = m.mall_id
       WHERE 1=1 ${src}
       ORDER BY e.exception_time DESC
    `).all();
  } catch (error) {
    if (/no such table/i.test(String(error?.message || ""))) return { rows: [] };
    throw error;
  }
  return { rows };
}
...
function buildActivityList(db, options = {}) {
  ...
  for (const r of out) {
    ...
    e.activities.push({ activity_id: r.activity_id, kind: r.kind, title: r.title, status: r.status,
      ...
      cost: r.cost, start_at: null, end_at: r.end_at, sites: [] });
  }
  ...
  const enrollDetails = optionalAllLocal(db, `
    ...
    SELECT e.mall_id, m.store_code, e.sku_ext_code, e.product_id, e.goods_id, e.skc_id,
           e.activity_thematic_id, e.activity_thematic_name, e.activity_type,
           ...
           sc.product_id AS spu_id, k.wac AS cost
      FROM cloud.temu_activity_enroll_record e
      JOIN le ON le.mall_id = e.mall_id AND le.sd = e.stat_date
      ...
      LEFT JOIN (SELECT internal_sku_code, MAX(COALESCE(NULLIF(weighted_avg_cost,0), NULLIF(jst_cost_price,0))) AS wac
  ...
  for (const er of enrollDetails) {
    ...
    if (!e) continue;
    const edk = `${pkey}|${er.activity_thematic_id || er.activity_thematic_name || ""}|${er.sku_ext_code}`;
    if (enrollMerged.has(edk)) continue; enrollMerged.add(edk);
    ...
    if (existing) {
      ...
      if (er.activity_stock != null && !existing.activity_stock) existing.activity_stock = Number(er.activity_stock);
      ...
      if (!existing.status) existing.status = enrollStatusLabel(er.enroll_status);
      ...
      e.activities.push({ activity_id: er.activity_thematic_id || null, kind: "enrolled", title: er.activity_thematic_name || null,
        status: enrollStatusLabel(er.enroll_status), activity_type: er.activity_type != null ? Number(er.activity_type) : null,
        sku_id: null, signup_price: sp, suggested_price: sg, price_diff: sp != null && sg != null ? sp - sg : null,
        activity_stock: er.activity_stock != null ? Number(er.activity_stock) : 0, cost, start_at: fmtEndTime(er.session_start_time), end_at: fmtEndTime(er.session_end_time), sites: sites || [] });
    }
  }
...
function getHighPriceFlowDetail(db, options = {}) {
  ...
  const sites = siteList.map((s) => ({ id: s.siteId, name: s.siteName }));
