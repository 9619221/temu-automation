...
```bash
curl -s -X POST http://localhost:19280 -H "Content-Type: application/json" -d '{"type":"get_status"}' --connect-timeout 3
```

- 如果返回响应（任何内容） → Worker 在线，跳到步骤 2
- 如果连接失败 → 执行步骤 1b
...
```bash
curl -s -X POST http://localhost:19280 -H "Content-Type: application/json" -d '{"type":"scrape_all"}' --max-time 1800
```
...
```bash
curl -s -X POST http://localhost:19280 -H "Content-Type: application/json" -d '{"type":"scrape_products"}' --max-time 300
```
...
| 浏览器未关闭 | `curl -s -X POST http://localhost:19280 -H "Content-Type: application/json" -d '{"type":"close_browser"}'` |
