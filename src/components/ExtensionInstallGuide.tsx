...
export default function ExtensionInstallGuide({ variant = "panel" }: ExtensionInstallGuideProps) {
  ...
  const openJushuitanOnce = async () => {
    try {
      await openExternalUrl("https://www.erp321.com/#temu-jst-collect-once=1");
      message.info("已打开聚水潭一次性采集链接，请在扩展弹窗确认采集窗口已开启。");
    } catch (error: any) {
      message.error(error?.message || "打开聚水潭采集链接失败");
    }
  };
    ...
    return (
      ...
      >
        ...
        <Text type="secondary" style={{ fontSize: 13 }}>
          需要安装 Chrome 扩展并打开 Temu 或聚水潭页面，手动触发后才会采集本次数据。
        </Text>
  ...
  return (
    ...
    >
      <div className="app-panel__title">
        <div>
          ...
          <div className="app-panel__title-sub">
            安装扩展后，在 Temu 后台或聚水潭页面打开采集窗口，本次接口数据会进入队列。
          </div>
        </div>
        <Space size={8} wrap>
          <Tag color={status.color} icon={status.icon} style={{ borderRadius: 999, margin: 0 }}>
      ...
      <div
        className="extension-install-guide__body"
        style={{
          display: "grid",
          gap: 16,
          alignItems: "start",
        ...
        <Space direction="vertical" size={12} style={{ width: "100%" }}>
          <Space size={8} wrap>
            ...
            <Tag icon={<ApiOutlined />} style={{ borderRadius: 999, padding: "3px 10px", margin: 0 }}>
              2 {hasPackageUrl ? "加载已解压扩展" : "打开采集页面"}
            </Tag>
          ...
          <Text type="secondary" style={{ lineHeight: 1.8 }}>
            {hasPackageUrl
              ...
              : hasStoreUrl
                ? "点击安装后会跳到 Chrome Web Store。安装完成后，打开 Temu 或聚水潭页面并刷新一次，通常 30 秒内这里会显示已连接。"
          ...
          <Space size={10} wrap>
            ...
            <Button icon={<ApiOutlined />} onClick={openJushuitanOnce}>
              打开聚水潭采集链接
            </Button>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div className="app-kv">
            ...
            <span className="app-kv__value">
              {newestAgent?.hook_xhr_alive === 1 ? "XHR 正常" : newestAgent?.hook_xhr_alive === 0 ? "XHR 异常" : "等待采集页面"}
            </span>
          </div>
