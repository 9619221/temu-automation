...
async function performInventoryActionCore(payload = {}, actorInput = {}) {const { db, services } = requireErp();const action = requireString(payload.action, "action");const actor = normalizeActor(actorInput);const { INVENTORY_LEDGER_TYPE } = require("./workflow/enums.cjs");switch (action) {case "purchase_return":{// 自家仓退给 1688 供应商。库存按 FIFO 扣，并按退货单价冲减 SKU 货值、重算剩余均价。
        ...
        LEFT JOIN erp_skus s ON s.id = l.sku_id
        WHERE l.source_doc_id = ? AND l.type IN ('sku_swap_out', 'sku_swap_in')
      `, [sourceDocId]);if (!legs.length) throw new Error(`换货单 ${sourceDocId} 不存在或已撤销`);const outLeg = legs.find((r) => r.type === "sku_swap_out");const inLeg = legs.find((r) => r.type === "sku_swap_in");if (!outLeg || !inLeg) throw new Error("换货流水不完整，缺少换出或换入记录");const fromSkuId = outLeg.sku_id;const toSkuId = inLeg.sku_id;const fromQty = Math.abs(Number(outLeg.qd) || 0);const toQty = Math.abs(Number(inLeg.qd) || 0);const fromUnitCost = Number(outLeg.uc) || 0;const fromAmount = fromQty * fromUnitCost;const fromAccountId = outLeg.account_id;const toAccountId = inLeg.account_id;if (!fromAccountId || !toAccountId) throw new Error("换货 SKU 缺少店铺绑定");const revertSourceDocId = `revert-${sourceDocId}`;return { action, ...(await withTransaction(db, async () => {const outLines = await services.inventory.applyDirectOutbound({ accountId: toAccountId, skuId: toSkuId, qty: toQty, unitCost: fromAmount / toQty, ledgerType: INVENTORY_LEDGER_TYPE.SKU_SWAP_OUT, sourceDocType: "revert_swap_sku", sourceDocId: revertSourceDocId, affectSkuTotal: false, actor });await services.inventory.adjustSkuInventoryValue(toSkuId, -toQty, -fromAmount);const inBatch = await services.inventory.applyDirectInbound({ accountId: fromAccountId, skuId: fromSkuId, qty: fromQty, unitLandedCost: fromUnitCost, ledgerType: INVENTORY_LEDGER_TYPE.SKU_SWAP_IN, sourceDocType: "revert_swap_sku", sourceDocId: revertSourceDocId, affectSkuTotal: false, actor });await services.inventory.adjustSkuInventoryValue(fromSkuId, fromQty, fromAmount);return { outLines, inBatch };})) };}case "consign_deliver_ship":{// 送仓托管行本地确认发货：按明细把货从本地仓扣掉（发到 Temu 仓）。
        ...
        return quickPullNewConsign({ db, force: payload.force === true }).then((r) => ({ action, ...r }));}case "consign_sku_advice":{// 按店+SKU 查平台建议备货量（erp_temu_openapi_sku_sales，15min 同步），供创建发货单弹窗展示。
        const mallId = requireString(payload.mallId || payload.mall_id, "mallId");const skuIds = (Array.isArray(payload.productSkuIds) ? payload.productSkuIds : []).map(String).filter(Boolean);if (!skuIds.length) return { action, rows: [] };const ph = skuIds.map(() => "?").join(",");return queryAll(db, `SELECT product_sku_id, advice_qty, lack_quantity FROM erp_temu_openapi_sku_sales WHERE mall_id = ? AND product_sku_id IN (${ph})`, [mallId, ...skuIds]).then((rows) => ({ action, rows: rows.map((r) => ({ productSkuId: String(r.product_sku_id), adviceQty: r.advice_qty == null ? null : Number(r.advice_qty), lackQuantity: r.lack_quantity == null ? null : Number(r.lack_quantity) })) }));}case "consign_official_ship_create":{// 创建官方发货单（生成 FH 单，可撤销、不真发货）。skuList=[{productSkuId,qty}]。
