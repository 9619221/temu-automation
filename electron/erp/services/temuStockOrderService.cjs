...
class TemuStockOrderService {
  ...
  async findMappedSkuId(accountId, skcId, skuId) {
    let row = null;
    if (skcId) {
      row = await queryOne(this.db, "SELECT id FROM erp_skus WHERE account_id = ? AND temu_skc_id = ?", [accountId, skcId]);
    }
    if (!row && skuId) {
      row = await queryOne(this.db, "SELECT id FROM erp_skus WHERE account_id = ? AND temu_sku_id = ?", [accountId, skuId]);
    }
    return row?.id || null;
  }

  async syncFromCollection({ accountId, orders, actor }) {
    const rows = Array.isArray(orders) ? orders : [];return await withTransaction(this.db,
      async (txDb) => {
        let synced = 0;
        let matched = 0;
        let unmatched = 0;
        ...
        for (const order of rows) {
          ...
          const mappedSkuId = await this.findMappedSkuId(accountId, skcId, skuId);
          const existing = await queryOne(txDb, `
        SELECT * FROM erp_temu_stock_orders
        WHERE account_id = ? AND temu_purchase_order_no = ?
      `, [accountId, purchaseOrderNo]);const syncStatus = existing?.sync_status === "outbound_created" ? "outbound_created" : mappedSkuId ? "matched" : "unmatched";
          const row = {
      ...
      `, [row]);}synced += 1;if (mappedSkuId) matched += 1;else unmatched += 1;}this.workflow.writeAudit({ accountId, actor, action: "temu_stock_order_sync", entityType: "temu_stock_order", entityId: null, before: null, after: { synced, matched, unmatched } });return { synced, matched, unmatched };});}async list({ accountId, status, limit }) {const params = [accountId];let sql = "SELECT * FROM erp_temu_stock_orders WHERE account_id = ?";if (status) {sql += " AND sync_status = ?";params.push(status);}
    sql += " ORDER BY updated_at DESC LIMIT ?";
    params.push(normalizeLimit(limit));
    return await queryAll(this.db, sql, [...params]);
  }

  async resolveOutboundTarget(stockOrderId) {
    const stockOrder = await queryOne(this.db, "SELECT * FROM erp_temu_stock_orders WHERE id = ?", [stockOrderId]);
    ...
    if (!stockOrder.mapped_erp_sku_id) {
      await execute(this.db, `
        ...
        WHERE id = @id
      `, { id: stockOrder.id, updated_at: nowIso() });
      throw new Error("Stock order not mapped to erp sku");
    }

    const batch = await queryOne(this.db, `
      ...
      LIMIT 1
    `, [stockOrder.account_id, stockOrder.mapped_erp_sku_id, stockOrder.demand_qty]);
