...
async function replaceSourceRecords(db, mallId, source, rows, now) {await withTransaction(db,
    ...
    async (txDb) => {
      await execute(txDb, "DELETE FROM erp_temu_openapi_records WHERE mall_id = ? AND source = ?", [mallId, source]);
  ...
  `, { mall_id: mallId, source, seq, record_key: row.record_key ?? null, product_id: row.product_id ?? null, product_skc_id: row.product_skc_id ?? null,
...
async function syncInventoryCollector(db, mallId, creds) {
  ...
  await Promise.all(Array.from({ length: INVENTORY_CONCURRENCY }, async () => await worker()));
  await replaceSourceRecords(db, mallId, "inventory", rows, nowIso());
  return rows.length;
}
...
async function syncAdReportMall(db, mallId, creds) {
  ...
  if (!body) {await replaceSourceRecords(db, mallId, "ad_report_mall", [], nowIso());return 0;}
  const row = {
  ...
  await replaceSourceRecords(db, mallId, "ad_report_mall", [row], nowIso());
  return 1;
}
...
async function syncAdReportGoods(db, mallId, creds, extraParams = {}) {
  ...
  try {
    ...
    await replaceSourceRecords(db, mallId, "ad_report_goods", [], nowIso());
    throw e;
  }
  const body = unwrapAdResult(response);
  ...
  await replaceSourceRecords(db, mallId, "ad_report_goods", rows, nowIso());
  return rows.length;
}
