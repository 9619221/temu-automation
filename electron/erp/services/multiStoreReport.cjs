...
// PG 模式 HTTP 回退：当 ATTACH 不可用时通过 cloud HTTP API 获取抓包事件
async function fetchCaptureEventsViaHttp(urlPaths) {
  ...
  if (resp.status === 401) {
    cachedToken = null;
    const newToken = await loginCloud(fetchImpl);
    resp = await fetchImpl(url, { headers: { Authorization: `Bearer ${newToken}` } });
  }
  if (!resp.ok) throw new Error(`settlement-capture HTTP ${resp.status}`);
...
function emptySettlementDetailBucket() {return { count: 0, estimated: 0, sales_receipt: 0, chargeback: 0, subsidy: 0, total: 0 };}function emptySettlementDetail() {return { currency: "CNY", wait_settlement: emptySettlementDetailBucket(), in_settlement: emptySettlementDetailBucket(), settled: emptySettlementDetailBucket() };}async function buildSettlementDetailByMall(db, opts = {}) {// 注意：不按 stat_date 过滤。汇总卡片的统计窗口由采集时的请求参数决定（面板「结算时间范围」），
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) await execute(txDb, `
    ...
    ON CONFLICT(scope_key, site, settlement_status, stat_date, item_key) DO UPDATE SET
      ...
      source = excluded.source,
      synced_at = excluded.synced_at
  `, { ...row, synced_at: now });});return { rows: rows.length };}async function syncSettlementDetailFromCapture(db, opts = {}) {await ensureSettlementDetailSchema(db);let events;try {events = await getSettlementCaptureEvents(db, SETTLEMENT_DETAIL_PATHS, opts);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}if (!events) return { ok: false, attached: false, malls: 0, rows: 0 };const rows = [];const malls = new Set();for (const ev of events) {const eventRows = buildSettlementDetailRowsFromCaptureEvent(ev);for (const row of eventRows) {rows.push(row);if (row.mall_id) malls.add(row.mall_id);}}const result = await upsertSettlementDetailRows(db, rows);return { ok: true, attached: true, malls: malls.size, rows: result.rows };} // =====================================================================
  ...
  `, [params]);else await execute(txDb, `
    ...
    VALUES
      (@mall_id, @trans_sn, @batch_id, @transaction_time, @create_time,
       ...
       @amount, @origin_amount, @remark, @remark_prompt,
       @biz_type, @source_region, @query_id, @site, @source, @now)
  `, [params]);inserted++;}});return { rows: inserted };}async function syncFundDetailFromCapture(db, opts = {}) {await ensureFundDetailSchema(db);let events;try {events = await getSettlementCaptureEvents(db, [FUND_DETAIL_PATH], opts);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}if (!events) return { ok: false, attached: false, malls: 0, rows: 0 };const rows = [];const malls = new Set();for (const ev of events) {const eventRows = buildFundDetailRowsFromCaptureEvent(ev);for (const r of eventRows) {rows.push(r);if (r.mall_id) malls.add(r.mall_id);}}if (!rows.length) return { ok: true, attached: true, malls: 0, rows: 0 };const result = await upsertFundDetailRows(db, rows);return { ok: true, attached: true, malls: malls.size, rows: result.rows };} // =====================================================================
...
const SITE_EXC_API_PATH = "/api/kiana/mms/robin/queryFullyOtherMessage";const SITE_EXC_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "097_sku_site_exceptions.sql");async function ensureSiteExceptionSchema(db) {await execSql(db, fs.readFileSync(SITE_EXC_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_sku_site_exceptions");}async function syncSiteExceptionsFromCapture(db, opts = {}) {ensureSiteExceptionSchema(db);let events;try {events = await getSettlementCaptureEvents(db, [SITE_EXC_API_PATH], opts);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}if (!events) return { ok: false, attached: false, malls: 0, rows: 0 };if (!events.length) return { ok: true, attached: true, malls: 0, rows: 0 };try {await execute(db, "DELETE FROM erp_sku_site_exceptions WHERE sku_id LIKE 'goods:%'");} catch {/* */}const skuGoodsMap = {};const goodsInfoMap = {};const skuInfoMap = {};try {const mappingEvents = await queryAll(db, `
...
async function buildPurchaseReport(db, options = {}) {
  ...
  const monthly = (await optionalAllLocal(db, `
    SELECT substr(created_at,1,7) AS mon, COUNT(*) count, SUM(${AMT}) amount
  ...
  map((r) => ({ month: r.mon, count: toNum(r.count), amount: toNum(r.amount) })).reverse();
  ...
  const cashMonthly = (await optionalAllLocal(db, `
    SELECT substr(paid_at,1,7) AS mon, COUNT(*) count, COALESCE(SUM(${AMT}),0) amount
...
async function buildWarehouseInventory(db, options = {}) {
  ...
  `SELECT
     ...
     GROUP BY b.account_id, acct.name
