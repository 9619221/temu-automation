...
const fs = require("fs");
const { queryAll, queryOne, execute, execSql, withTransaction, getTableColumns, tableExists } = require("../../db/connection.cjs");
...
async function cloudTableColumns(db, tableName) {
  try {
    return await getTableColumns(db, tableName);
  } catch {
    return [];
...
async function cloudTableExists(db, tableName) {
  try {
...
async function addColumnIfMissing(db, tableName, columnName, definition) {
  ...
  await execSql(db, `ALTER TABLE ${tableName} ADD COLUMN ${definition}`);
}

async function ensureSourceColumn(db, tableName) {
  await addColumnIfMissing(db, tableName, "source", `source TEXT NOT NULL DEFAULT '${SETTLEMENT_UNKNOWN_SOURCE}'`);
  await execSql(db, `CREATE INDEX IF NOT EXISTS idx_${tableName}_source ON ${tableName}(source)`);
}

async function sourceWhere(db, tableName, alias = "") {
  const columns = await tableColumns(db, tableName);
...
async function robotCaptureSql(db, alias = "ce") {
  const eventColumns = await cloudTableColumns(db, "capture_events");
  ...
  if (await cloudTableExists(db, "devices")) {
    return {
...
async function getToken(fetchImpl) {
  ...
  return await loginCloud(fetchImpl);
}
...
async function readMallDictionary(db) {
  try {
...
async function optionalAllLocal(db, sql, params = []) {
  try {
...
async function upsertSettlementIncomeFromDashboard(db, payload = {}) {
  ...
  await ensureSourceColumn(db, "erp_temu_settlement_income");
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) await execute(txDb, `
  ...
  );return { rows: rows.length, scopeKey: rows[0].scope_key, mallId: rows[0].mall_id, accountId: rows[0].account_id };} // 看板财务接口路径（官方开放平台无财务/结算 API，只能走 worker/扩展抓包，原始事件落 cloud.capture_events）
const SETTLEMENT_INCOME_PATH = "/api/merchant/front/finance/income-summary";const SETTLEMENT_INCOME_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "081_temu_settlement_income.sql");async function ensureSettlementIncomeSchema(db) {await execSql(db, fs.readFileSync(SETTLEMENT_INCOME_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_settlement_income");}function extractSettlementIncomeListFromCaptureBody(body) {const candidates = [body?.result, body?.result?.data, body?.result?.list, body?.result?.rows, body?.result?.items, body?.result?.records, body?.result?.incomeSummaryList, body?.data?.result, body?.data?.data, body?.data?.list, body?.data?.rows, body?.data?.items, body?.data?.records, body?.data?.incomeSummaryList, body?.data, body?.list, body?.rows, body?.items, body?.records, body?.incomeSummaryList, body];return candidates.find((value) => Array.isArray(value)) || [];} // 采集入口：从 cloud.capture_events 批量物化结算收入。抓包按店、按日合并（received_at 升序、
...
async function syncSettlementIncomeFromCapture(db, opts = {}) {await ensureSettlementIncomeSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");events = await queryAll(db, `
    ...
    `, [SETTLEMENT_INCOME_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;} // 按 mall 聚合：stat_date → 原始 income item（received_at 升序遍历，后写覆盖=最新抓包）
  ...
  `, [since, latest]);const d7start = shiftDate(latest, -6);const d7prevStart = shiftDate(latest, -13);const d7prevEnd = shiftDate(latest, -7);const d30start = shiftDate(latest, -29);const d30prevStart = shiftDate(latest, -59);const d30prevEnd = shiftDate(latest, -30);const byMall = new Map();const ensure = (mallId) => {if (!byMall.has(mallId)) {const settlement = emptySettlement();settlement.latest_date = latest;settlement._trend = new Map();byMall.set(mallId, settlement);}return byMall.get(mallId);};for (const row of rows) {const settlement = ensure(row.mall_id);const income = Number(row.income) || 0;const d = row.d;settlement._trend.set(d, { date: d, income });if (d === latest) settlement.today.income += income;if (d >= d7start && d <= latest) settlement.last7d.income += income;if (d >= d7prevStart && d <= d7prevEnd) settlement.last7d.income_prev += income;if (d >= d30start && d <= latest) settlement.last30d.income += income;if (d >= d30prevStart && d <= d30prevEnd) settlement.last30d.income_prev += income;}for (const settlement of byMall.values()) {settlement.last7d.income_wow = settlement.last7d.income_prev > 0 ? (settlement.last7d.income - settlement.last7d.income_prev) / settlement.last7d.income_prev : null;settlement.last30d.income_mom = settlement.last30d.income_prev > 0 ? (settlement.last30d.income - settlement.last30d.income_prev) / settlement.last30d.income_prev : null;settlement.trend_daily = Array.from(settlement._trend.values()).sort((a, b) => a.date.localeCompare(b.date));delete settlement._trend;}return byMall;} // 结算明细三态（待处理/结算中/已到账）按店聚合，来源 erp_temu_settlement_detail（抓包物化）。
...
function emptySettlementDetailBucket() {return { count: 0, estimated: 0, sales_receipt: 0, chargeback: 0, subsidy: 0, total: 0 };}function emptySettlementDetail() {return { currency: "CNY", wait_settlement: emptySettlementDetailBucket(), in_settlement: emptySettlementDetailBucket(), settled: emptySettlementDetailBucket() };}async function buildSettlementDetailByMall(db, opts = {}) {// 注意：不按 stat_date 过滤。汇总卡片的统计窗口由采集时的请求参数决定（面板「结算时间范围」），
  ...
  const { startDate, endDate } = opts;const params = [];let dateWhere = "";if (startDate && endDate) {dateWhere = "AND stat_date >= ? AND stat_date <= ?";params.push(startDate, endDate);}let rows;try {const sourceSql = sourceWhere(db, "erp_temu_settlement_detail"); // settle/detail/full/* 接口返回的是「汇总卡片」（一段时间范围一个总数），每次采集落一行快照。
    ...
    `, [...params]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return null;throw error;}if (!rows.length) return new Map();const byMall = new Map();for (const row of rows) {if (!byMall.has(row.mall_id)) byMall.set(row.mall_id, emptySettlementDetail());const detail = byMall.get(row.mall_id);const bucket = detail[row.settlement_status];if (!bucket) continue; // 未知状态忽略
    bucket.count = Number(row.cnt) || 0;bucket.estimated = Number(row.estimated) || 0;bucket.sales_receipt = Number(row.sales_receipt) || 0;bucket.chargeback = Number(row.chargeback) || 0;bucket.subsidy = Number(row.subsidy) || 0;bucket.total = Number(row.total) || 0;if (row.currency) detail.currency = row.currency;}return byMall;}const SETTLEMENT_DETAIL_PATH_STATUS = new Map([["/api/merchant/settle/detail/full/wait-settlement", "wait_settlement"], ["/api/merchant/settle/detail/full/in-settlement", "in_settlement"], ["/api/merchant/settle/detail/full/settled", "settled"]]);const SETTLEMENT_DETAIL_PATHS = Array.from(SETTLEMENT_DETAIL_PATH_STATUS.keys());const SETTLEMENT_DETAIL_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "082_temu_settlement_detail.sql");async function ensureSettlementDetailSchema(db) {await execSql(db, fs.readFileSync(SETTLEMENT_DETAIL_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_settlement_detail");}function bjDateFromTimestamp(value) {const n = Number(value);const ms = Number.isFinite(n) && n > 0 ? n : Date.now();return new Date(ms + 8 * 3600000).toISOString().slice(0, 10);}function normalizeSettlementDetailDate(item, receivedAt) {return normalizeSettlementDate(item) || bjDateFromTimestamp(receivedAt);}function stableJsonHash(value) {return crypto.createHash("sha1").update(JSON.stringify(value || {})).digest("hex").slice(0, 16);}function extractSettlementDetailListFromCaptureBody(body) {const candidates = [body?.result?.data?.list, body?.result?.data?.rows, body?.result?.data?.items, body?.result?.list, body?.result?.rows, body?.result?.items, body?.data?.result?.list, body?.data?.result?.rows, body?.data?.list, body?.data?.rows, body?.data?.items, body?.list, body?.rows, body?.items];const list = candidates.find((value) => Array.isArray(value));if (Array.isArray(list)) return list;const objectCandidate = body?.result?.data || body?.result || body?.data || body;return objectCandidate && typeof objectCandidate === "object" ? [objectCandidate] : [];}function normalizeMoneyAmount(value) {if (value == null || value === "") return null;if (typeof value === "object") {const cents = Number(value.amount ?? value.cent ?? value.cents);if (Number.isFinite(cents)) {return { yuan: cents / 100, cents: Math.round(cents), currency: value.currencyCode || value.currency || null };} // digitalText 可能带千分位逗号（"3,359.43"），直接 Number() 会 NaN（028 实测金额>999 全被解析成 0）
    const text = value.digitalText ?? value.fullText ?? value.text;let yuanObj = typeof text === "number" ? text : NaN;if (typeof text === "string") {const n = Number(text.replace(/[,\s¥$€£]/g, ""));if (Number.isFinite(n)) yuanObj = n;} // text 字段缺失时退到 value.value（接口原值，单位分；digitalText 语义是绝对值，保持一致取 abs）
    if (!Number.isFinite(yuanObj) && Number.isFinite(Number(value.value))) {yuanObj = Math.abs(Number(value.value)) / 100;}if (Number.isFinite(yuanObj)) {return { yuan: yuanObj, cents: Math.round(yuanObj * 100), currency: value.currencyCode || value.currency || null };}return null;}const yuan = Number(String(value).replace(/[,\s]/g, "").replace(/^[^\d.-]+/, ""));if (!Number.isFinite(yuan)) return null;return { yuan, cents: Math.round(yuan * 100), currency: null };}function collectSettlementAmounts(root) {const out = {};const stack = [{ value: root, path: "" }];let steps = 0;const amountKeyRe = /(amount|income|payment|receipt|reverse|reversal|chargeback|subsidy|settlement|sales|refund|compensat|allowance)/i;while (stack.length && steps < 3000) {steps++;const { value, path: p } = stack.pop();if (!value || typeof value !== "object") continue;if (Array.isArray(value)) {value.forEach((item, idx) => stack.push({ value: item, path: `${p}[${idx}]` }));continue;}for (const [key, child] of Object.entries(value)) {const childPath = p ? `${p}.${key}` : key;if (amountKeyRe.test(key)) {const parsed = normalizeMoneyAmount(child);if (parsed) out[childPath] = parsed;}if (child && typeof child === "object") stack.push({ value: child, path: childPath });}}return out;}function firstAmountByPath(amounts, patterns, rejectPatterns = []) {for (const [key, parsed] of Object.entries(amounts || {})) {if (rejectPatterns.some((pattern) => pattern.test(key))) continue;if (patterns.some((pattern) => pattern.test(key))) return parsed;}return null;}function pickSettlementDetailItemKey(item) {const explicit = firstDefined(item, ["settlementOrderSn", "settlementSn", "settlementNo", "settlementId", "billNo", "billId", "detailId", "id", "orderSn", "transactionId"]);return explicit ? String(explicit) : `raw_${stableJsonHash(item)}`;}function buildSettlementDetailRowsFromCaptureEvent(ev) {const status = SETTLEMENT_DETAIL_PATH_STATUS.get(ev.url_path);if (!status) return [];let body;try {body = JSON.parse(ev.body_json);} catch {return [];}const list = extractSettlementDetailListFromCaptureBody(body);const mallId = String(ev.mall_id || "").trim();if (!mallId || !list.length) return [];const site = String(ev.site || "").trim();const scopeKey = mallId;return list.filter((item) => item && typeof item === "object").map((item) => {const amounts = collectSettlementAmounts(item); // Temu 实际字段名：waitSettleAmount / incomeAmount / afsReverseAmount / afsReleaseAmount
    const estimated = firstAmountByPath(amounts, [/estimate/i, /waitSettle/i, /wait.*settle/i, /pending/i]);const sales = firstAmountByPath(amounts, [/sales/i, /receipt/i, /payment/i, /^incomeAmount/i, /^income/i], [/chargeback/i, /reverse/i, /reversal/i, /refund/i]);const chargeback = firstAmountByPath(amounts, [/chargeback/i, /reverse/i, /reversal/i, /refund/i]);const subsidy = firstAmountByPath(amounts, [/subsidy/i, /compensat/i, /allowance/i, /afsRelease/i, /release/i]);const total = firstAmountByPath(amounts, [/total/i, /^amount$/i, /settlementAmount/i], [/estimate/i, /wait/i, /pending/i]);const currency = estimated?.currency || sales?.currency || chargeback?.currency || subsidy?.currency || total?.currency || item.currency || "CNY";const derivedTotal = total?.yuan ?? (sales?.yuan || 0) - (chargeback?.yuan || 0) + (subsidy?.yuan || 0);return { scope_key: scopeKey, mall_id: mallId, site, settlement_status: status, stat_date: normalizeSettlementDetailDate(item, ev.received_at), item_key: pickSettlementDetailItemKey(item), currency: String(currency || "CNY"), estimated_amount: estimated?.yuan || 0, sales_receipt_amount: sales?.yuan || 0, chargeback_amount: chargeback?.yuan || 0, subsidy_amount: subsidy?.yuan || 0, total_amount: Number.isFinite(derivedTotal) ? derivedTotal : 0, amounts_json: JSON.stringify(amounts), raw_json: JSON.stringify(item), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE };});}async function upsertSettlementDetailRows(db, rows) {if (!rows.length) return { rows: 0 };const now = new Date().toISOString();
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) await execute(txDb, `
    ...
    ON CONFLICT(scope_key, site, settlement_status, stat_date, item_key) DO UPDATE SET
      ...
      source = excluded.source,
      synced_at = excluded.synced_at
  `, { ...row, synced_at: now });});return { rows: rows.length };}async function syncSettlementDetailFromCapture(db, opts = {}) {await ensureSettlementDetailSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");const siteSelect = capture.columns.includes("site") ? "ce.site" : "'' AS site";events = await queryAll(db, `
      SELECT ce.mall_id, ${siteSelect}, ce.url_path, ce.body_json, ce.received_at
       ...
       WHERE ce.url_path IN (${SETTLEMENT_DETAIL_PATHS.map(() => "?").join(",")})
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [...SETTLEMENT_DETAIL_PATHS]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}const rows = [];const malls = new Set();for (const ev of events) {const eventRows = buildSettlementDetailRowsFromCaptureEvent(ev);for (const row of eventRows) {rows.push(row);if (row.mall_id) malls.add(row.mall_id);}}const result = await upsertSettlementDetailRows(db, rows);return { ok: true, attached: true, malls: malls.size, rows: result.rows };} // =====================================================================
...
// =====================================================================
const FUND_DETAIL_PATH = "/api/merchant/fund/detail/pageSearch";async function ensureFundDetailSchema(db) {const migSql = fs.readFileSync(path.join(__dirname, "..", "..", "db", "migrations", "083_temu_fund_detail.sql"), "utf8");await execSql(db, migSql);await ensureSourceColumn(db, "erp_temu_fund_detail");}function buildFundDetailRowsFromCaptureEvent(ev) {let body;try {body = JSON.parse(ev.body_json);} catch {return [];}let list = body?.result?.resultList || body?.result?.list || body?.resultList || body?.list || [];if (!Array.isArray(list)) return [];const mallId = String(ev.mall_id || "").trim();if (!mallId) return [];const site = String(ev.site || "").trim() || "kuajingmaihuo";return list.filter((item) => item && typeof item === "object").map((item) => {let amount = null;if (item.amountFormat && typeof item.amountFormat.value === "number") {amount = item.amountFormat.value;} else if (typeof item.amount === "number") {amount = item.amount;} else if (typeof item.amount === "string") {const m = item.amount.match(/[+-]?[\d,]+\.?\d*/);if (m) amount = parseFloat(m[0].replace(/,/g, ""));}if (amount != null && item.moneyChangeType === 2 && amount > 0) amount = -amount;let originAmount = null;if (item.originAmount != null) {if (typeof item.originAmount === "number") originAmount = item.originAmount;else if (typeof item.originAmount === "string") {const om = item.originAmount.match(/[+-]?[\d,]+\.?\d*/);if (om) originAmount = parseFloat(om[0].replace(/,/g, ""));}}return { mall_id: mallId, trans_sn: item.transSn || item.transactionSn || null, batch_id: item.batchId || "", transaction_time: item.transactionTime || item.createTime || "", create_time: item.createTime || "", money_change_type: item.moneyChangeType ?? null, fund_type: item.fundType ?? null, fund_type_desc: item.fundTypeDesc || "", currency: item.currencyType || item.amountFormat?.currencyCode || "CNY", amount, origin_amount: originAmount, remark: item.remark || "", remark_prompt: typeof item.remarkPrompt === "string" ? item.remarkPrompt : item.remarkPrompt ? JSON.stringify(item.remarkPrompt) : "", biz_type: item.bizType != null ? String(item.bizType) : "", source_region: item.sourceRegion != null ? String(item.sourceRegion) : "", query_id: item.queryId || "", site, source: SETTLEMENT_ROBOT_SOURCE };});}async function upsertFundDetailRows(db, rows) {if (!rows.length) return { rows: 0 };const now = new Date().toISOString();let inserted = 0;
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) {const params = { ...row, now };if (row.trans_sn) await execute(txDb, `
  ...
  `, [params]);else await execute(txDb, `
    INSERT INTO erp_temu_fund_detail
      (mall_id, trans_sn, batch_id, transaction_time, create_time,
       money_change_type, fund_type, fund_type_desc, currency,
       amount, origin_amount, remark, remark_prompt,
    ...
    VALUES
      (@mall_id, @trans_sn, @batch_id, @transaction_time, @create_time,
       ...
       @amount, @origin_amount, @remark, @remark_prompt,
       @biz_type, @source_region, @query_id, @site, @source, @now)
  `, [params]);inserted++;}});return { rows: inserted };}async function syncFundDetailFromCapture(db, opts = {}) {await ensureFundDetailSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");const siteSelect = capture.columns.includes("site") ? "ce.site" : "'' AS site";events = await queryAll(db, `
      SELECT ce.mall_id, ${siteSelect}, ce.url_path, ce.body_json, ce.received_at
        FROM ${capture.from}
       WHERE ce.url_path = ?
         AND ce.mall_id IS NOT NULL AND ce.mall_id <> ''
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [FUND_DETAIL_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}const rows = [];const malls = new Set();for (const ev of events) {const eventRows = buildFundDetailRowsFromCaptureEvent(ev);for (const r of eventRows) {rows.push(r);if (r.mall_id) malls.add(r.mall_id);}}if (!rows.length) return { ok: true, attached: true, malls: 0, rows: 0 };const result = await upsertFundDetailRows(db, rows);return { ok: true, attached: true, malls: malls.size, rows: result.rows };} // =====================================================================
...
// =====================================================================
const SETTLEMENT_ORDER_DETAIL_PATH = "/api/merchant/fund/detail/item/semi/download";const SETTLEMENT_ORDER_DETAIL_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "085_temu_settlement_order_detail.sql");async function ensureSettlementOrderDetailSchema(db) {await execSql(db, fs.readFileSync(SETTLEMENT_ORDER_DETAIL_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_settlement_order_detail");const columns = await getTableColumns(db, "erp_temu_settlement_order_detail");if (!columns.includes("wb_no")) {await execSql(db, "ALTER TABLE erp_temu_settlement_order_detail ADD COLUMN wb_no TEXT");}await execSql(db, "CREATE INDEX IF NOT EXISTS idx_temu_settlement_order_detail_wb ON erp_temu_settlement_order_detail(wb_no)");}function normalizeHeaderName(value) {return String(value ?? "").trim().toLowerCase().replace(/[\s_\-:：/\\()[\]{}【】（）"'`]+/g, "");}function pickRowValue(row, keys, opts = {}) {if (!row || typeof row !== "object") return null;for (const key of keys) {const value = row[key];if (value !== undefined && value !== null && value !== "") return value;}const normalized = new Map();for (const [key, value] of Object.entries(row)) {if (value === undefined || value === null || value === "") continue;normalized.set(normalizeHeaderName(key), value);}for (const key of keys) {const nk = normalizeHeaderName(key);if (normalized.has(nk)) return normalized.get(nk);}if (opts.includes) {const wanted = keys.map(normalizeHeaderName).filter(Boolean);for (const [key, value] of normalized.entries()) {if (wanted.some((needle) => key.includes(needle))) return value;}}return null;}function parseNumberLoose(value) {if (value == null || value === "") return null;if (typeof value === "number") return Number.isFinite(value) ? value : null;if (typeof value === "object") {return parseNumberLoose(firstDefined(value, ["value", "amount", "digitalText", "fullText", "text"]));}const text = String(value).trim();if (!text) return null;const negativeByParen = /^\(.*\)$/.test(text);const normalized = text.replace(/,/g, "").replace(/[^\d.+-]/g, "");if (!normalized || normalized === "-" || normalized === "+") return null;const n = Number(normalized);if (!Number.isFinite(n)) return null;return negativeByParen && n > 0 ? -n : n;}function parseIntegerLoose(value) {const n = parseNumberLoose(value);return Number.isFinite(n) ? Math.round(n) : null;}function normalizeWbNo(value) {const text = String(value ?? "").trim();if (!text) return null;const match = text.match(/\bWB[A-Z0-9-]+\b/i);return match ? match[0].toUpperCase() : null;}function pickSettlementWbNo(row) {const direct = pickRowValue(row, ["wb_no", "wbNo", "WB", "WB No", "WB No.", "stockUpOrderNo", "stock_up_order_no", "purchaseOrderNo", "purchase_order_no", "备货单号", "备货单编号", "备货单"], { includes: true });const directWb = normalizeWbNo(direct);if (directWb) return directWb;if (!row || typeof row !== "object") return null;for (const value of Object.values(row)) {if (value && typeof value === "object") {const nested = normalizeWbNo(firstDefined(value, ["value", "text", "fullText", "digitalText"]));if (nested) return nested;continue;}const found = normalizeWbNo(value);if (found) return found;}return null;}function extractSettlementOrderDetailRowsFromCaptureBody(body) {const meta = body?.result?.data || body?.result || body?.data || body || {};const candidates = [meta?.rows, meta?.list, meta?.items, meta?.records, body?.rows, body?.list, body?.items, body?.records];const rows = candidates.find((value) => Array.isArray(value)) || [];const columns = Array.isArray(meta?.columns) ? meta.columns : rows[0] && typeof rows[0] === "object" ? Object.keys(rows[0]) : [];const sourceRowCount = parseIntegerLoose(meta?.rowCount ?? meta?.total ?? meta?.totalCount) ?? rows.length;return { rows, columns, meta, sourceRowCount };}function buildSettlementOrderDetailRowsFromCaptureEvent(ev) {let body;try {body = JSON.parse(ev.body_json);} catch {return [];}const { rows, columns, meta, sourceRowCount } = extractSettlementOrderDetailRowsFromCaptureBody(body);const mallId = String(ev.mall_id || "").trim();if (!mallId || !rows.length) return [];const batchId = String(firstDefined(meta, ["batchId", "batch_id", "itemBizId", "item_biz_id"]) || `capture_${stableJsonHash(meta)}`);const site = String(ev.site || meta.site || "").trim() || "agentseller";const columnsJson = JSON.stringify(columns || []);return rows.filter((row) => row && typeof row === "object").map((row, index) => {const currency = pickRowValue(row, ["币种", "货币", "currency", "currencyCode"]) || meta.currency || "CNY";return { mall_id: mallId, site, batch_id: batchId, fund_type: firstDefined(meta, ["fundType", "fund_type"]) != null ? String(firstDefined(meta, ["fundType", "fund_type"])) : null, create_time: String(firstDefined(meta, ["createTime", "create_time", "transactionTime", "transaction_time"]) || ""), sheet_name: String(firstDefined(meta, ["sheetName", "sheet_name"]) || ""), source_row_count: sourceRowCount, row_index: index + 1, wb_no: pickSettlementWbNo(row), order_sn: pickRowValue(row, ["订单号", "订单编号", "子订单号", "orderSn", "order_sn", "orderNo", "order_no"]), parent_order_sn: pickRowValue(row, ["父订单号", "父单号", "parentOrderSn", "parent_order_sn", "parentOrderNo"]), sku_id: pickRowValue(row, ["SKU ID", "商品SKU ID", "商品 SKU ID", "skuId", "sku_id", "productSkuId"]), sku_ext_code: pickRowValue(row, ["商家SKU", "商家 SKU", "SKU货号", "SKU 货号", "货号", "skuExtCode", "sku_ext_code", "skuCode", "sellerSku", "externalSku"]), product_name: pickRowValue(row, ["商品名称", "商品标题", "品名", "productName", "product_name", "goodsName"]), quantity: parseNumberLoose(pickRowValue(row, ["数量", "商品数量", "件数", "quantity", "qty"])), currency: String(currency || "CNY"), amount: parseNumberLoose(pickRowValue(row, ["结算金额", "应结算金额", "实收金额", "入账金额", "总金额", "金额", "settlementAmount", "settlement_amount", "totalAmount", "amount"], { includes: true })), columns_json: columnsJson, raw_json: JSON.stringify(row), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE };});}async function upsertSettlementOrderDetailRows(db, rows) {if (!rows.length) return { rows: 0 };const now = new Date().toISOString();
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) await execute(txDb, `
    ...
    ON CONFLICT(mall_id, batch_id, row_index) DO UPDATE SET
      ...
      source = excluded.source,
      updated_at = excluded.updated_at
  `, { ...row, now });});return { rows: rows.length };}async function syncSettlementOrderDetailFromCapture(db, opts = {}) {await ensureSettlementOrderDetailSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");const siteSelect = capture.columns.includes("site") ? "ce.site" : "'' AS site";events = await queryAll(db, `
      SELECT ce.mall_id, ${siteSelect}, ce.url_path, ce.body_json, ce.received_at
        FROM ${capture.from}
       WHERE ce.url_path = ?
         AND ce.mall_id IS NOT NULL AND ce.mall_id <> ''
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [SETTLEMENT_ORDER_DETAIL_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}const rows = [];const malls = new Set();for (const ev of events) {const eventRows = buildSettlementOrderDetailRowsFromCaptureEvent(ev);for (const row of eventRows) {rows.push(row);if (row.mall_id) malls.add(row.mall_id);}}if (!rows.length) return { ok: true, attached: true, malls: 0, rows: 0 };const result = await upsertSettlementOrderDetailRows(db, rows);return { ok: true, attached: true, malls: malls.size, rows: result.rows };} // =====================================================================
// 资金/账务汇总（seller.kuajingmaihuo.com fund/detail day/month summary）
// =====================================================================
const FUND_SUMMARY_PATH_SCOPE = new Map([["/api/merchant/fund/detail/daySummary", "day"], ["/api/merchant/fund/detail/monthSummary", "month"]]);const FUND_SUMMARY_PATHS = Array.from(FUND_SUMMARY_PATH_SCOPE.keys());const FUND_SUMMARY_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "086_temu_fund_summary.sql");async function ensureFundSummarySchema(db) {await execSql(db, fs.readFileSync(FUND_SUMMARY_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_fund_summary");}function extractFundSummaryListFromCaptureBody(body) {const candidates = [body?.result, body?.result?.data, body?.result?.data?.list, body?.result?.data?.rows, body?.result?.data?.items, body?.result?.list, body?.result?.rows, body?.result?.items, body?.data?.result?.list, body?.data?.result?.rows, body?.data?.list, body?.data?.rows, body?.data?.items, body?.list, body?.rows, body?.items];const list = candidates.find((value) => Array.isArray(value));if (Array.isArray(list)) return list;const objectCandidate = body?.result?.data || body?.result || body?.data || body;return objectCandidate && typeof objectCandidate === "object" ? [objectCandidate] : [];}function normalizeFundSummaryDate(item, scope, receivedAt) {const raw = firstDefined(item, ["date", "statDate", "stat_date", "dateStr", "dataDate", "day", "summaryDate", "month", "statMonth", "stat_month", "yearMonth", "billMonth", "billingCycle"]);const text = raw == null ? "" : String(raw).trim();if (scope === "month") {const month = text.match(/\d{4}[-/]\d{1,2}/);if (month) {const [y, m] = month[0].replace(/\//g, "-").split("-");return `${y}-${String(m).padStart(2, "0")}`;}const day = normalizeSettlementDate(item);if (day) return day.slice(0, 7);return bjDateFromTimestamp(receivedAt).slice(0, 7);}return normalizeSettlementDate(item) || bjDateFromTimestamp(receivedAt);}function parseFundSummaryMetric(value) {if (value && typeof value === "object") {const money = normalizeMoneyAmount(value);if (money) return money;}const yuan = parseNumberLoose(value);if (!Number.isFinite(yuan)) return null;return { yuan, cents: Math.round(yuan * 100), currency: null };}function collectFundSummaryMetrics(root) {const out = {};const stack = [{ value: root, path: "" }];const keyRe = /(amount|balance|income|expense|payment|receipt|frozen|freeze|available|total|sum|fund|money|余额|金额|收入|支出|流入|流出|冻结|限制|可用|合计|扣款|费用)/i;let steps = 0;while (stack.length && steps < 3000) {steps++;const { value, path: p } = stack.pop();if (!value || typeof value !== "object") continue;if (Array.isArray(value)) {value.forEach((item, idx) => stack.push({ value: item, path: `${p}[${idx}]` }));continue;}for (const [key, child] of Object.entries(value)) {const childPath = p ? `${p}.${key}` : key;if (keyRe.test(key)) {const parsed = parseFundSummaryMetric(child);if (parsed) out[childPath] = parsed;}if (child && typeof child === "object") stack.push({ value: child, path: childPath });}}return out;}function buildFundSummaryRowsFromCaptureEvent(ev) {const scope = FUND_SUMMARY_PATH_SCOPE.get(ev.url_path);if (!scope) return [];let body;try {body = JSON.parse(ev.body_json);} catch {return [];}const list = extractFundSummaryListFromCaptureBody(body);const mallId = String(ev.mall_id || "").trim();if (!mallId || !list.length) return [];const site = String(ev.site || "").trim() || "kuajingmaihuo";return list.filter((item) => item && typeof item === "object").map((item) => {const metrics = collectFundSummaryMetrics(item);const income = firstAmountByPath(metrics, [/income/i, /receipt/i, /inAmount/i, /收入/, /流入/], [/refund/i, /frozen/i, /freeze/i, /balance/i, /available/i]);const expense = firstAmountByPath(metrics, [/expense/i, /expenditure/i, /outAmount/i, /payment/i, /支出/, /流出/, /扣款/, /费用/], [/balance/i, /available/i]);const balance = firstAmountByPath(metrics, [/balance/i, /account/i, /余额/], [/available/i, /frozen/i, /freeze/i]);const frozen = firstAmountByPath(metrics, [/frozen/i, /freeze/i, /restrict/i, /limit/i, /hold/i, /冻结/, /限制/]);const available = firstAmountByPath(metrics, [/available/i, /usable/i, /withdraw/i, /可用/, /可提现/]);const total = firstAmountByPath(metrics, [/total/i, /sum/i, /^amount$/i, /合计/, /总/], [/count/i, /page/i]);const currency = income?.currency || expense?.currency || balance?.currency || frozen?.currency || available?.currency || total?.currency || item.currency || item.currencyType || "CNY";return { mall_id: mallId, site, summary_scope: scope, summary_date: normalizeFundSummaryDate(item, scope, ev.received_at), currency: String(currency || "CNY"), income_amount: income?.yuan ?? 0, expense_amount: expense?.yuan ?? 0, balance_amount: balance?.yuan ?? null, frozen_amount: frozen?.yuan ?? null, available_amount: available?.yuan ?? null, total_amount: total?.yuan ?? null, metrics_json: JSON.stringify(metrics), raw_json: JSON.stringify(item), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE };});}async function upsertFundSummaryRows(db, rows) {if (!rows.length) return { rows: 0 };const now = new Date().toISOString();
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) await execute(txDb, `
    ...
    ON CONFLICT(mall_id, site, summary_scope, summary_date) DO UPDATE SET
      ...
      source = excluded.source,
      updated_at = excluded.updated_at
  `, { ...row, now });});return { rows: rows.length };}async function syncFundSummaryFromCapture(db, opts = {}) {await ensureFundSummarySchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");const siteSelect = capture.columns.includes("site") ? "ce.site" : "'' AS site";events = await queryAll(db, `
      SELECT ce.mall_id, ${siteSelect}, ce.url_path, ce.body_json, ce.received_at
        FROM ${capture.from}
    ...
    `, [...params]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();for (const r of rows) {if (!byMall.has(r.mall_id)) {byMall.set(r.mall_id, { in_total: 0, out_total: 0, by_category: {} });}const m = byMall.get(r.mall_id);const amt = Number(r.total_amount) || 0;if (r.money_change_type === 1) m.in_total += amt;else if (r.money_change_type === 2) m.out_total += amt;m.by_category[r.category] = (m.by_category[r.category] || 0) + amt;}return byMall;} // ===== 结算报表独立查询（支持自定义时间段） =====
async function buildFundSummaryByMall(db, opts = {}) {const { startDate, endDate } = opts;let where = "";let params = [];if (startDate && endDate) {where = `
      WHERE (
    ...
    `, [...params]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();const ensure = (mallId) => {if (!byMall.has(mallId)) {byMall.set(mallId, { latest_date: null, currency: "CNY", income_total: 0, expense_total: 0, balance_amount: 0, frozen_amount: 0, available_amount: 0, total_amount: 0, rows: 0, day_rows: 0, month_rows: 0, _month_income_total: 0, _month_expense_total: 0, _latest_key: "", _latest_balance_key: "", _latest_frozen_key: "", _latest_available_key: "", _latest_total_key: "" });}return byMall.get(mallId);};const setLatestAmount = (out, row, field, latestField, latestKey) => {if (row[field] == null) return;const value = Number(row[field]);if (!Number.isFinite(value)) return;if (!out[latestField] || latestKey >= out[latestField]) {out[field] = value;out[latestField] = latestKey;}};for (const row of rows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;const out = ensure(mallId);out.rows += 1;if (row.summary_scope === "day") {out.day_rows += 1;out.income_total += Number(row.income_amount) || 0;out.expense_total += Number(row.expense_amount) || 0;} else if (row.summary_scope === "month") {out.month_rows += 1;out._month_income_total += Number(row.income_amount) || 0;out._month_expense_total += Number(row.expense_amount) || 0;}const latestKey = `${row.summary_date || ""}|${String(row.source_received_at || 0).padStart(16, "0")}|${String(row.id || 0).padStart(10, "0")}`;if (!out._latest_key || latestKey >= out._latest_key) {out._latest_key = latestKey;out.latest_date = row.summary_date || null;out.currency = row.currency || out.currency || "CNY";}setLatestAmount(out, row, "balance_amount", "_latest_balance_key", latestKey);setLatestAmount(out, row, "frozen_amount", "_latest_frozen_key", latestKey);setLatestAmount(out, row, "available_amount", "_latest_available_key", latestKey);setLatestAmount(out, row, "total_amount", "_latest_total_key", latestKey);}for (const out of byMall.values()) {if (out.day_rows === 0 && out.month_rows > 0) {out.income_total = out._month_income_total;out.expense_total = out._month_expense_total;}delete out._month_income_total;delete out._month_expense_total;delete out._latest_key;delete out._latest_balance_key;delete out._latest_frozen_key;delete out._latest_available_key;delete out._latest_total_key;}return byMall;} // ============ EPR 费用（087）：eprfee goods/platform 抓包物化 ============
const EPR_FEE_GOODS_PATH = "/api/merchant/eprfee/goods/page-query";const EPR_FEE_PLATFORM_PATH = "/api/merchant/eprfee/platform/wait-deduction/page-query";const EPR_FEE_PACKAGE_PATH = "/api/merchant/eprfee/package/query";const EPR_FEE_EXPORT_PATH = "/api/merchant/file/export/history/page";const EPR_FEE_PATHS = [EPR_FEE_GOODS_PATH, EPR_FEE_PLATFORM_PATH, EPR_FEE_PACKAGE_PATH, EPR_FEE_EXPORT_PATH];const EPR_FEE_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "087_temu_epr_fee.sql"); // goods 响应里四个列表各对应一种扣款状态
const EPR_GOODS_LIST_STATUS = [["waitDeductionEprFeeInfoList", "wait"], ["deductedEprFeeInfoList", "deducted"], ["waitRefundEprFeeInfoList", "wait_refund"], ["refundedEprFeeInfoList", "refunded"]];async function ensureEprFeeSchema(db) {await execSql(db, fs.readFileSync(EPR_FEE_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_epr_fee");}function buildEprFeeRow(item, { mallId, site, feeScope, deductStatus, receivedAt }) {// 字段名暂无非空真实样本，按 enum/summary 接口字段推测 + 宽容提取；raw_json 全留供校准
  const amountRaw = firstDefined(item, ["amount", "feeAmount", "deductAmount", "totalAmount", "eprFeeAmount", "deductionAmount", "payAmount", "金额", "扣费金额", "已扣费", "费用", "账单金额", "扣款金额", "应付金额", "实付金额"]);const amount = amountRaw && typeof amountRaw === "object" ? normalizeMoneyAmount(amountRaw)?.yuan ?? parseNumberLoose(amountRaw) : parseNumberLoose(amountRaw);const originalRaw = firstDefined(item, ["originalAmount", "originAmount", "totalOriginalAmount"]);const originalAmount = originalRaw && typeof originalRaw === "object" ? normalizeMoneyAmount(originalRaw)?.yuan ?? parseNumberLoose(originalRaw) : parseNumberLoose(originalRaw);const currency = amountRaw && typeof amountRaw === "object" && amountRaw.currencyCode || item.currency || item.currencyCode || item.alphabetCode || "CNY";const explicitKey = firstDefined(item, ["id", "billId", "feeId", "certId", "recordId", "detailId", "skuId", "productSkuId", "单据号", "账单号", "流水号", "订单号", "商品ID", "SKU ID", "SKU"]);return { mall_id: mallId, site, fee_scope: feeScope, deduct_status: deductStatus, item_key: explicitKey != null && explicitKey !== "" ? String(explicitKey) : `raw_${stableJsonHash(item)}`, cert_type: toNullableText(firstDefined(item, ["certType", "certCode", "证书类型", "资质类型"])), cert_name: toNullableText(firstDefined(item, ["certName", "certDisplayName", "证书名称", "资质名称", "EPR名称", "费用名称", "项目"])), region: toNullableText(firstDefined(item, ["regionName", "regionCode", "region", "siteName", "区域", "站点", "国家/地区", "国家"])), sku_id: toNullableText(firstDefined(item, ["skuId", "productSkuId", "prodSkuId", "SKU ID", "SKU", "sku"])), spu_id: toNullableText(firstDefined(item, ["spuId", "productId", "goodsId", "商品ID", "商品 Id", "SPU ID"])), goods_name: toNullableText(firstDefined(item, ["goodsName", "productName", "skuName", "title", "商品名称", "品名"])), quantity: parseNumberLoose(firstDefined(item, ["quantity", "totalQuantity", "qty", "num", "数量", "件数"])), amount: Number.isFinite(amount) ? amount : 0, original_amount: Number.isFinite(originalAmount) ? originalAmount : null, currency: String(currency || "CNY"), stat_date: normalizeSettlementDetailDate(item, receivedAt), raw_json: JSON.stringify(item), source_received_at: Number(receivedAt) || null, source: SETTLEMENT_ROBOT_SOURCE };}function toNullableText(value) {if (value == null) return null;const text = String(value).trim();return text ? text.slice(0, 500) : null;}function getEprExportMeta(body) {const taskType = Number(body?.taskType ?? body?.result?.taskType);const type = String(body?.type || body?.result?.type || "");if (taskType === 18 || type === "Epr_Product") return { feeScope: "goods", deductStatus: "deducted" };if (taskType === 16 || type === "Epr_Proxy") return { feeScope: "proxy", deductStatus: "deducted" };if (taskType === 21 || type === "Epr_Agency") return { feeScope: "agency", deductStatus: "deducted" };return { feeScope: "export", deductStatus: "deducted" };}function buildEprFeeRowsFromCaptureEvent(ev) {let body;try {body = JSON.parse(ev.body_json);} catch {return [];}const mallId = String(ev.mall_id || "").trim();if (!mallId) return [];const site = String(ev.site || "").trim() || "agentseller";const result = body?.result || {};const rows = [];if (ev.url_path === EPR_FEE_EXPORT_PATH) {const exportRows = Array.isArray(body?.rows) ? body.rows : Array.isArray(result?.rows) ? result.rows : [];const meta = getEprExportMeta(body);for (const item of exportRows) {if (!item || typeof item !== "object") continue;rows.push(buildEprFeeRow({ ...item, __juxieyun_type: body.type || result.type || "", __juxieyun_task_type: body.taskType || result.taskType || "", __juxieyun_file_name: body.fileName || result.fileName || "", __juxieyun_area: body.area || result.area || "" }, { mallId, site, feeScope: meta.feeScope, deductStatus: meta.deductStatus, receivedAt: ev.received_at }));}} else if (ev.url_path === EPR_FEE_GOODS_PATH) {for (const [listKey, deductStatus] of EPR_GOODS_LIST_STATUS) {const list = result[listKey];if (!Array.isArray(list)) continue;for (const item of list) {if (!item || typeof item !== "object") continue;rows.push(buildEprFeeRow(item, { mallId, site, feeScope: "goods", deductStatus, receivedAt: ev.received_at }));}}} else if (ev.url_path === EPR_FEE_PLATFORM_PATH) {const list = Array.isArray(result.dataList) ? result.dataList : [];for (const item of list) {if (!item || typeof item !== "object") continue;rows.push(buildEprFeeRow(item, { mallId, site, feeScope: "platform", deductStatus: "wait", receivedAt: ev.received_at }));}} else if (ev.url_path === EPR_FEE_PACKAGE_PATH) {// 包裹级：响应列表名无真实样本，按 goods 四列表 + dataList/list 通用提取，落 fee_scope='package'。
    // item_key 优先显式 id、否则按内容 hash 去重，多列表并存不会重复落同一条。
    const pkgLists = [...EPR_GOODS_LIST_STATUS, ["dataList", "wait"], ["list", "wait"]];for (const [listKey, deductStatus] of pkgLists) {const list = result[listKey];if (!Array.isArray(list)) continue;for (const item of list) {if (!item || typeof item !== "object") continue;rows.push(buildEprFeeRow(item, { mallId, site, feeScope: "package", deductStatus, receivedAt: ev.received_at }));}}}return rows;}async function upsertEprFeeRows(db, rows) {if (!rows.length) return { rows: 0 };const now = new Date().toISOString();
  ...
  await withTransaction(db, async (txDb) => {for (const row of rows) await execute(txDb, `
    ...
    ON CONFLICT(mall_id, fee_scope, deduct_status, item_key) DO UPDATE SET
      ...
      source = excluded.source,
      synced_at = excluded.synced_at
  `, { ...row, now });});return { rows: rows.length };}async function syncEprFeeFromCapture(db, opts = {}) {await ensureEprFeeSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");const siteSelect = capture.columns.includes("site") ? "ce.site" : "'' AS site";events = await queryAll(db, `
      SELECT ce.mall_id, ${siteSelect}, ce.url_path, ce.body_json, ce.received_at
        FROM ${capture.from}
       WHERE ce.url_path IN (${EPR_FEE_PATHS.map(() => "?").join(",")})
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [...EPR_FEE_PATHS]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}const rows = [];const malls = new Set();for (const ev of events) {for (const row of buildEprFeeRowsFromCaptureEvent(ev)) {rows.push(row);malls.add(row.mall_id);}}if (!rows.length) return { ok: true, attached: true, malls: 0, rows: 0 };const result = await upsertEprFeeRows(db, rows);return { ok: true, attached: true, malls: malls.size, rows: result.rows };}async function buildEprFeeByMall(db) {let rows;try {const sourceSql = sourceWhere(db, "erp_temu_epr_fee");rows = await queryAll(db, `
      SELECT mall_id, deduct_status, SUM(amount) AS total_amount, COUNT(*) AS cnt
        FROM erp_temu_epr_fee
       WHERE 1=1
         ${sourceSql}
       GROUP BY mall_id, deduct_status
    `);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();for (const row of rows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;if (!byMall.has(mallId)) {byMall.set(mallId, { wait_amount: 0, deducted_amount: 0, wait_count: 0, deducted_count: 0, total_count: 0 });}const out = byMall.get(mallId);const amount = Number(row.total_amount) || 0;const cnt = Number(row.cnt) || 0;if (row.deduct_status === "wait" || row.deduct_status === "wait_refund") {out.wait_amount += amount;out.wait_count += cnt;} else {out.deducted_amount += amount;out.deducted_count += cnt;}out.total_count += cnt;}return byMall;} // ============ 资金限制（088）：fund-frozen/rules 抓包物化（快照语义） ============
const FUND_FROZEN_PATH = "/api/merchant/fund-frozen/rules";const FUND_FROZEN_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "088_temu_fund_frozen.sql");async function ensureFundFrozenSchema(db) {await execSql(db, fs.readFileSync(FUND_FROZEN_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_fund_frozen");}function buildFundFrozenRowsFromCaptureEvent(ev) {let body;try {body = JSON.parse(ev.body_json);} catch {return [];}const mallId = String(ev.mall_id || "").trim();if (!mallId) return [];const result = body?.result || {};const rules = Array.isArray(result.rules) ? result.rules : [];const fallbackCurrency = result.currency || "CNY";return rules.filter((item) => item && typeof item === "object").map((item, index) => ({ mall_id: mallId, frozen_type: toNullableText(item.frozenType) || `unknown_${index}`, reason: toNullableText(item.reason), // amount 源是 "￥7.40" 字符串，parseNumberLoose 剥货币符号/逗号
      amount: parseNumberLoose(firstDefined(item, ["amount", "currentAmount", "dr2Amount"])) ?? 0, currency: String(item.currency || fallbackCurrency || "CNY"), unfreeze_condition: toNullableText(item.unfreezeCondition), description: toNullableText(item.description), raw_json: JSON.stringify(item), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE }));}async function syncFundFrozenFromCapture(db, opts = {}) {await ensureFundFrozenSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");events = await queryAll(db, `
      SELECT ce.mall_id, ce.body_json, ce.received_at
        FROM ${capture.from}
       WHERE ce.url_path = ?
         AND ce.mall_id IS NOT NULL AND ce.mall_id <> ''
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [FUND_FROZEN_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;} // 快照语义：每店只取最新一次抓包（received_at 升序后写覆盖），落库前清掉该店旧行，
  // 避免已解冻项残留虚高冻结额
  const latestByMall = new Map();for (const ev of events) latestByMall.set(String(ev.mall_id), ev);const now = new Date().toISOString();let total = 0;const malls = new Set();
  ...
  await withTransaction(db, async (txDb) => {for (const [mallId, ev] of latestByMall) {const rows = buildFundFrozenRowsFromCaptureEvent(ev);await execute(txDb, `DELETE FROM erp_temu_fund_frozen WHERE mall_id = ?${sourceWhere(db, "erp_temu_fund_frozen")}`, [mallId]);for (const row of rows) {await execute(txDb, `
    ...
    ON CONFLICT(mall_id, frozen_type) DO UPDATE SET
      ...
      source = excluded.source,
      synced_at = excluded.synced_at
  `, { ...row, now });total++;}if (rows.length) malls.add(mallId);}}
    ...
    `);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();for (const row of rows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;if (!byMall.has(mallId)) byMall.set(mallId, { total_amount: 0, items: [] });const out = byMall.get(mallId);out.total_amount += Number(row.amount) || 0;out.items.push({ frozen_type: row.frozen_type, reason: row.reason, amount: Number(row.amount) || 0, currency: row.currency, unfreeze_condition: row.unfreeze_condition });}return byMall;} // ============ 账户概览（090）：payment/account/amount/info 抓包物化（每店一行快照） ============
const ACCOUNT_OVERVIEW_PATH = "/api/merchant/payment/account/amount/info";const ACCOUNT_OVERVIEW_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "090_temu_account_overview.sql");async function ensureAccountOverviewSchema(db) {await execSql(db, fs.readFileSync(ACCOUNT_OVERVIEW_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_account_overview");} // 金额宽容提取：对象走 normalizeMoneyAmount（分→元），字符串/数字走 parseNumberLoose（按元）
function extractMoneyYuan(obj, keys) {const raw = firstDefined(obj, keys);if (raw == null) return 0;if (typeof raw === "object") {const n = normalizeMoneyAmount(raw)?.yuan ?? parseNumberLoose(raw);return Number.isFinite(n) ? n : 0;}const n = parseNumberLoose(raw);return Number.isFinite(n) ? n : 0;}function buildAccountOverviewRow(ev) {let body;try {body = JSON.parse(ev.body_json);} catch {return null;}const mallId = String(ev.mall_id || "").trim();if (!mallId) return null;const r = body?.result;if (!r || typeof r !== "object") return null;const currencyRaw = firstDefined(r, ["currency", "currencyCode", "alphabetCode"]);return { mall_id: mallId, available_amount: extractMoneyYuan(r, ["availableAmount", "availableBalance", "available", "canUseAmount"]), in_transit_amount: extractMoneyYuan(r, ["inTransitAmount", "onTheWayAmount", "transitAmount", "onWayAmount"]), pending_settle_amount: extractMoneyYuan(r, ["pendingSettlementAmount", "waitSettleAmount", "toBeSettledAmount", "unsettledAmount", "pendingAmount"]), frozen_amount: extractMoneyYuan(r, ["frozenAmount", "freezeAmount", "frozen"]), withdrawable_amount: extractMoneyYuan(r, ["withdrawableAmount", "canWithdrawAmount", "withdrawAmount"]), total_amount: extractMoneyYuan(r, ["totalAmount", "accountAmount", "balanceAmount", "balance", "total"]), currency: String(currencyRaw || "CNY"), raw_json: JSON.stringify(r), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE };}async function syncAccountOverviewFromCapture(db, opts = {}) {await ensureAccountOverviewSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");events = await queryAll(db, `
      SELECT ce.mall_id, ce.body_json, ce.received_at
        FROM ${capture.from}
       WHERE ce.url_path = ?
         AND ce.mall_id IS NOT NULL AND ce.mall_id <> ''
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [ACCOUNT_OVERVIEW_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;} // 快照语义：每店取最新一次抓包覆盖
  const latestByMall = new Map();for (const ev of events) latestByMall.set(String(ev.mall_id), ev);const now = new Date().toISOString();let total = 0;const malls = new Set();
  ...
  await withTransaction(db, async (txDb) => {for (const [mallId, ev] of latestByMall) {const row = buildAccountOverviewRow(ev);if (!row) continue;await execute(txDb, `
  ...
  `, { ...row, now });total++;malls.add(mallId);}}
      ...
      SELECT mall_id, available_amount, in_transit_amount, pending_settle_amount, frozen_amount,
       ...
       WHERE 1=1
         ${sourceSql}
    `);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();for (const row of rows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;byMall.set(mallId, { available_amount: Number(row.available_amount) || 0, in_transit_amount: Number(row.in_transit_amount) || 0, pending_settle_amount: Number(row.pending_settle_amount) || 0, frozen_amount: Number(row.frozen_amount) || 0, withdrawable_amount: Number(row.withdrawable_amount) || 0, total_amount: Number(row.total_amount) || 0, currency: row.currency || "CNY" });}return byMall;} // ============ 履约费用流出（091）：warehouse/express/bill overview+detail 抓包物化 ============
const FULFILLMENT_OVERVIEW_PATH = "/api/merchant/warehouse/express/bill/global/overview";const FULFILLMENT_DETAIL_PATH = "/api/merchant/warehouse/express/bill/detail/list";const FULFILLMENT_PATHS = [FULFILLMENT_OVERVIEW_PATH, FULFILLMENT_DETAIL_PATH];const FULFILLMENT_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "091_temu_fulfillment_bill.sql");async function ensureFulfillmentBillSchema(db) {await execSql(db, fs.readFileSync(FULFILLMENT_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_fulfillment_bill");}function buildFulfillmentDetailRow(item, { mallId, receivedAt }) {const explicitKey = firstDefined(item, ["billId", "id", "waybillNo", "trackingNumber", "recordId", "detailId", "ptransId"]);return { mall_id: mallId, record_type: "detail", item_key: explicitKey != null && explicitKey !== "" ? String(explicitKey) : `raw_${stableJsonHash(item)}`, bill_type: toNullableText(firstDefined(item, ["billType", "feeType", "chargeType", "expenseType", "type"])), amount: extractMoneyYuan(item, ["amount", "feeAmount", "chargeAmount", "totalAmount", "expressFee", "billAmount"]), currency: String(firstDefined(item, ["currency", "currencyCode"]) || "CNY"), waybill_no: toNullableText(firstDefined(item, ["waybillNo", "trackingNumber", "expressNo"])), stat_date: normalizeSettlementDetailDate(item, receivedAt), raw_json: JSON.stringify(item), source_received_at: Number(receivedAt) || null, source: SETTLEMENT_ROBOT_SOURCE };}function buildFulfillmentOverviewRow(result, { mallId, receivedAt }) {return { mall_id: mallId, record_type: "overview", item_key: "_overview", bill_type: null, amount: extractMoneyYuan(result, ["totalWaitPayAmount", "totalAmount", "totalFee", "totalExpense", "amount", "billTotalAmount", "sumAmount"]), currency: String(firstDefined(result, ["currency", "currencyCode"]) || "CNY"), waybill_no: null, stat_date: null, raw_json: JSON.stringify(result), source_received_at: Number(receivedAt) || null, source: SETTLEMENT_ROBOT_SOURCE };}async function syncFulfillmentBillFromCapture(db, opts = {}) {await ensureFulfillmentBillSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");events = await queryAll(db, `
      SELECT ce.mall_id, ce.url_path, ce.body_json, ce.received_at
       ...
       WHERE ce.url_path IN (${FULFILLMENT_PATHS.map(() => "?").join(",")})
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [...FULFILLMENT_PATHS]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}const now = new Date().toISOString(); // overview：每店取最新覆盖；detail：清掉该店旧明细后重灌（快照语义，避免历史明细累积虚高）
  const latestOverview = new Map();const detailEventsByMall = new Map();for (const ev of events) {const mid = String(ev.mall_id);if (ev.url_path === FULFILLMENT_OVERVIEW_PATH) {latestOverview.set(mid, ev);} else {if (!detailEventsByMall.has(mid)) detailEventsByMall.set(mid, []);detailEventsByMall.get(mid).push(ev);}}let total = 0;const malls = new Set();
  ...
  await withTransaction(db, async (txDb) => {for (const [mallId, ev] of latestOverview) {let body;try {body = JSON.parse(ev.body_json);} catch {continue;}const r = body?.result;if (!r || typeof r !== "object") continue;await execute(txDb, `
  ...
  `, { ...buildFulfillmentDetailRow(item, { mallId, receivedAt: ev.received_at }), now });total++;malls.add(mallId);}}}}
    ...
    `, [...params]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();for (const row of rows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;if (!byMall.has(mallId)) byMall.set(mallId, { overview_amount: 0, detail_total: 0, detail_count: 0, currency: "CNY", items: [] });const out = byMall.get(mallId);if (row.currency) out.currency = row.currency;if (row.record_type === "overview") {out.overview_amount = Number(row.amount) || 0;} else {out.detail_total += Number(row.amount) || 0;out.detail_count += 1;if (out.items.length < 200) {out.items.push({ bill_type: row.bill_type, amount: Number(row.amount) || 0, currency: row.currency, waybill_no: row.waybill_no, stat_date: row.stat_date });}}}return byMall;} // ============ 违规处罚明细（089）：tmod_punish entrance/list + island summary 物化 ============
const VIOLATION_LIST_PATH = "/mms/tmod_punish/agent/merchant_appeal/entrance/list";const VIOLATION_SUMMARY_PATH = "/mms/island/punish/summary";const VIOLATION_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "089_temu_violation.sql");async function ensureViolationSchema(db) {await execSql(db, fs.readFileSync(VIOLATION_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_temu_violation");await ensureSourceColumn(db, "erp_temu_violation_summary");}function buildViolationRowsFromCaptureEvent(ev) {let body;try {body = JSON.parse(ev.body_json);} catch {return [];}const mallId = String(ev.mall_id || "").trim();if (!mallId) return [];const list = body?.result?.punish_appeal_entrance_list;if (!Array.isArray(list)) return [];return list.filter((item) => item && typeof item === "object").map((item) => ({ mall_id: mallId, target_id: item.target_id != null ? String(item.target_id) : `raw_${stableJsonHash(item)}`, target_type: toNullableText(item.target_type), goods_id: item.goods_id != null ? String(item.goods_id) : null, spu_id: item.spu_id != null ? String(item.spu_id) : null, goods_name: toNullableText(item.goods_name), goods_img_url: toNullableText(item.goods_img_url), source_punish_name: toNullableText(item.source_punish_name), leaf_reason_name: toNullableText(item.leaf_reason_name), violation_desc: toNullableText(item.violation_desc), punish_status_desc: toNullableText(item.punish_status_desc), appeal_status: parseIntegerLoose(item.appeal_status), can_appeal: item.can_not_appeal == null ? null : item.can_not_appeal ? 0 : 1, can_rectify: item.can_rectify == null ? null : item.can_rectify ? 1 : 0, site_num: parseIntegerLoose(item.site_num), punish_num: parseIntegerLoose(item.punish_num), stat_date: bjDateFromTimestamp(ev.received_at), raw_json: JSON.stringify(item), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE }));}async function syncViolationFromCapture(db, opts = {}) {await ensureViolationSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {const capture = await robotCaptureSql(db, "ce");events = await queryAll(db, `
      SELECT ce.mall_id, ce.url_path, ce.body_json, ce.received_at
        FROM ${capture.from}
       WHERE ce.url_path IN (?, ?)
         ...
         ${capture.where}
       ORDER BY ce.received_at ASC
    `, [VIOLATION_LIST_PATH, VIOLATION_SUMMARY_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}const now = new Date().toISOString();let total = 0;const malls = new Set();
  ...
  await withTransaction(db, async (txDb) => {for (const ev of events) {const mallId = String(ev.mall_id).trim();if (ev.url_path === VIOLATION_LIST_PATH) {for (const row of buildViolationRowsFromCaptureEvent(ev)) {await execute(txDb, `
    ...
    ON CONFLICT(mall_id, target_id) DO UPDATE SET
      ...
      source = excluded.source,
      synced_at = excluded.synced_at
  `, { ...row, now });total++;malls.add(mallId);}} else if (ev.url_path === VIOLATION_SUMMARY_PATH) {let body;try {body = JSON.parse(ev.body_json);} catch {continue;}const result = body?.result;if (!result || typeof result !== "object") continue;await execute(txDb, `
  ...
  `, { mall_id: mallId, violation_count: parseIntegerLoose(result.violationCount) ?? 0, add_site_limit_status: parseIntegerLoose(result.addSiteLimitStatus), release_limit_time: toNullableText(result.releaseLimitTime), raw_json: JSON.stringify(result), source_received_at: Number(ev.received_at) || null, source: SETTLEMENT_ROBOT_SOURCE, now });total++;malls.add(mallId);}}});return { ok: true, attached: true, malls: malls.size, rows: total };}async function buildViolationByMall(db, opts = {}) {const detailLimit = Number(opts.detailLimit) || 50;const { startDate, endDate } = opts;const params = [];let detailWhere = "";const hasDateRange = !!(startDate && endDate);if (hasDateRange) {detailWhere = "WHERE stat_date >= ? AND stat_date <= ?";params.push(startDate, endDate);}let detailRows, summaryRows;try {const detailSourceSql = sourceWhere(db, "erp_temu_violation");const summarySourceSql = sourceWhere(db, "erp_temu_violation_summary");detailRows = await queryAll(db, `
    ...
    `, [...params]);summaryRows = hasDateRange ? [] : await queryAll(db, `
      SELECT mall_id, violation_count, add_site_limit_status, release_limit_time
        FROM erp_temu_violation_summary
       WHERE 1=1
         ${summarySourceSql}
    `);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();const ensure = (mallId) => {if (!byMall.has(mallId)) {byMall.set(mallId, { violation_count: 0, add_site_limit_status: null, release_limit_time: null, items: [] });}return byMall.get(mallId);};for (const row of detailRows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;const out = ensure(mallId);out.violation_count++;if (out.items.length < detailLimit) {out.items.push({ target_id: row.target_id, goods_id: row.goods_id, goods_name: row.goods_name, source_punish_name: row.source_punish_name, leaf_reason_name: row.leaf_reason_name, violation_desc: row.violation_desc, punish_status_desc: row.punish_status_desc, appeal_status: row.appeal_status, can_appeal: row.can_appeal, can_rectify: row.can_rectify, site_num: row.site_num, punish_num: row.punish_num, stat_date: row.stat_date });}}for (const row of summaryRows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;const out = ensure(mallId);out.add_site_limit_status = row.add_site_limit_status;out.release_limit_time = row.release_limit_time; // summary 的总数比明细全（明细可能只采了前几页），取大者
    out.violation_count = Math.max(out.violation_count, Number(row.violation_count) || 0);}return byMall;} // ============ SKU 站点异常（097）：queryFullyOtherMessage → cloud snapshot 物化 ============
const SITE_EXC_API_PATH = "/api/kiana/mms/robin/queryFullyOtherMessage";const SITE_EXC_MIGRATION = path.join(__dirname, "..", "..", "db", "migrations", "097_sku_site_exceptions.sql");async function ensureSiteExceptionSchema(db) {await execSql(db, fs.readFileSync(SITE_EXC_MIGRATION, "utf8"));await ensureSourceColumn(db, "erp_sku_site_exceptions");}async function syncSiteExceptionsFromCapture(db, opts = {}) {ensureSiteExceptionSchema(db);const attachCloudDb = opts.attachCloudDb;if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return { ok: false, attached: false, malls: 0, rows: 0 };}let events;try {events = await queryAll(db, `
      SELECT ce.mall_id, ce.url_path, ce.body_json, ce.received_at
        FROM cloud.capture_events ce
       WHERE ce.url_path = ?
         AND ce.mall_id IS NOT NULL AND ce.mall_id <> ''
         AND ce.body_json IS NOT NULL AND ce.body_json <> ''
       ORDER BY ce.received_at ASC
    `, [SITE_EXC_API_PATH]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) {return { ok: false, attached: true, malls: 0, rows: 0 };}throw error;}if (!events.length) return { ok: true, attached: true, malls: 0, rows: 0 };try {await execute(db, "DELETE FROM erp_sku_site_exceptions WHERE sku_id LIKE 'goods:%'");} catch {/* */}const skuGoodsMap = {};const goodsInfoMap = {};const skuInfoMap = {};try {const mappingEvents = await queryAll(db, `
      SELECT body_json FROM cloud.capture_events
       WHERE url_path = '/_internal/sku-goods-mapping'
         AND body_json IS NOT NULL AND body_json <> ''
    `);for (const me of mappingEvents) {try {const m = JSON.parse(me.body_json);if (!m || typeof m !== "object") continue;const sgm = m.skuGoodsMap || (m.goodsInfo ? null : m);if (sgm && typeof sgm === "object") Object.assign(skuGoodsMap, sgm);if (m.goodsInfo && typeof m.goodsInfo === "object") Object.assign(goodsInfoMap, m.goodsInfo);if (m.skuInfo && typeof m.skuInfo === "object") Object.assign(skuInfoMap, m.skuInfo);} catch {/* */}}} catch {/* */}const now = new Date().toISOString();let total = 0;const malls = new Set();await withTransaction(db, async (txDb) => {for (const ev of events) {let body;try {body = JSON.parse(ev.body_json);} catch {continue;}const mallId = String(ev.mall_id).trim();const result = body?.result;if (!result || typeof result !== "object") continue;const staticMap = {};const statics = result.fullyBindSiteFailVO?.staticDescVOList;if (Array.isArray(statics)) {for (const s of statics) if (s.checkCode != null) staticMap[s.checkCode] = s.checkDesc || "";} // SKU 级绑定站点失败：fullyBindSiteFailVO.goodsSkuBindSiteFailVOList
        const skuFailList = result.fullyBindSiteFailVO?.goodsSkuBindSiteFailVOList;if (Array.isArray(skuFailList)) {for (const sku of skuFailList) {if (!sku || !sku.goodsSkuId) continue;const skuId = String(sku.goodsSkuId);const siteInfos = sku.goodsSkuBindSiteFailInfoVOList;if (!Array.isArray(siteInfos)) continue;for (const si of siteInfos) {const siteName = String(si.siteName || "").trim();if (!siteName) continue;let exceptionTime = null;if (si.checkAt) {try {exceptionTime = new Date(Number(si.checkAt)).toISOString();} catch {/* */}}const fails = Array.isArray(si.failResultVOList) ? si.failResultVOList : [];const codes = fails.map((f) => String(f.checkCode)).join(",");const strip = (s) => s ? s.replace(/<[^>]*>/g, "").replace(/\s{2,}/g, " ").trim() : s;const descs = fails.map((f) => strip(f.checkDesc || staticMap[f.checkCode] || "")).filter(Boolean);const uniqueDescs = [...new Set(descs)].join("; ");await execute(txDb, `
    INSERT INTO erp_sku_site_exceptions
      (mall_id, sku_id, site_name, goods_id, skc_id, check_code,
       exception_reason, exception_time, sku_spec, raw_json,
       source, source_received_at, synced_at)
    VALUES
      (@mall_id, @sku_id, @site_name, @goods_id, @skc_id, @check_code,
       @exception_reason, @exception_time, @sku_spec, @raw_json,
       @source, @source_received_at, @now)
    ON CONFLICT(mall_id, sku_id, site_name) DO UPDATE SET
      goods_id = excluded.goods_id,
      skc_id = excluded.skc_id,
      check_code = excluded.check_code,
      exception_reason = excluded.exception_reason,
      exception_time = excluded.exception_time,
  ...
  `, { mall_id: mallId, sku_id: skuId, site_name: siteName, goods_id: skuGoodsMap[skuId] || null, skc_id: null, check_code: codes || null, exception_reason: uniqueDescs || null, exception_time: exceptionTime, sku_spec: skuInfoMap[skuId]?.spec || null, raw_json: JSON.stringify(sku), source: SETTLEMENT_ROBOT_SOURCE, source_received_at: Number(ev.received_at) || null, now });total++;malls.add(mallId);}}}}});return { ok: true, attached: true, malls: malls.size, rows: total };}async function buildSiteExceptionList(db, opts = {}) {await ensureSiteExceptionSchema(db);let rows;try {const src = sourceWhere(db, "erp_sku_site_exceptions");rows = await queryAll(db, `
      SELECT e.mall_id, e.sku_id, e.site_name, e.goods_id, e.skc_id,
             ...
             oa.ext_code AS sku_ext_code
        FROM erp_sku_site_exceptions e
        LEFT JOIN erp_temu_malls m ON e.mall_id = m.mall_id
        LEFT JOIN erp_temu_openapi_skus oa ON e.sku_id = oa.product_sku_id
       WHERE 1=1 ${src}
       ORDER BY e.exception_time DESC
    `);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return { rows: [], goodsInfo: {}, skuInfo: {} };throw error;}let goodsInfo = {};let skuInfo = {};try {const mappingEvents = await queryAll(db, `
      SELECT body_json FROM cloud.capture_events
       WHERE url_path = '/_internal/sku-goods-mapping'
         AND body_json IS NOT NULL AND body_json <> ''
    `);for (const me of mappingEvents) {try {const m = JSON.parse(me.body_json);if (m?.goodsInfo && typeof m.goodsInfo === "object") Object.assign(goodsInfo, m.goodsInfo);if (m?.skuInfo && typeof m.skuInfo === "object") Object.assign(skuInfo, m.skuInfo);} catch {/* */}}} catch {/* */} // 从官方 API 销量表补图片(capture_events 里 sku-goods-mapping 覆盖率低)
  try {const gids = new Set(rows.map((r) => r.goods_id || r.sku_id).filter(Boolean));if (gids.size > 0) {const thumbRows = await queryAll(db, `
      ...
      `);for (const tr of thumbRows) {for (const gid of [tr.product_id, tr.product_skc_id]) {if (!gid || !gids.has(gid)) continue;if (!goodsInfo[gid]) goodsInfo[gid] = {};if (!goodsInfo[gid].thumb) goodsInfo[gid].thumb = tr.thumb_url;if (!goodsInfo[gid].title && tr.title) goodsInfo[gid].title = tr.title;}}}} catch {/* */} // 通过 sku_id → product_sku_id 补图片（goods_id 与 product_id 不一致时的兜底）
  try {const missing = rows.filter((r) => {const gid = r.goods_id || r.sku_id;return gid && (!goodsInfo[gid] || !goodsInfo[gid].thumb);});if (missing.length > 0) {const skuToGid = new Map();for (const r of missing) skuToGid.set(r.sku_id, r.goods_id || r.sku_id);const skuIds = [...skuToGid.keys()];const batchSize = 500;for (let i = 0; i < skuIds.length; i += batchSize) {const batch = skuIds.slice(i, i + batchSize);const ph = batch.map(() => "?").join(",");const thumbRows = await queryAll(db, `
          SELECT product_sku_id, title, thumb_url
            FROM erp_temu_openapi_sku_sales
           WHERE product_sku_id IN (${ph})
             AND thumb_url IS NOT NULL AND thumb_url <> ''
        `, [...batch]);for (const tr of thumbRows) {const gid = skuToGid.get(tr.product_sku_id);if (!gid) continue;if (!goodsInfo[gid]) goodsInfo[gid] = {};if (!goodsInfo[gid].thumb) goodsInfo[gid].thumb = tr.thumb_url;if (!goodsInfo[gid].title && tr.title) goodsInfo[gid].title = tr.title;}}}} catch {/* */}return { rows, goodsInfo, skuInfo };}async function buildSettlementRiskByMall(db, opts = {}) {const { startDate, endDate } = opts;let where = "WHERE risk_type IN ('violation_goods', 'inbound_exception')";const params = [];if (startDate && endDate) {where += " AND stat_date >= ? AND stat_date <= ?";params.push(startDate, endDate);}let rows;try {rows = await queryAll(db, `
    ...
    `, [...params]);} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return new Map();throw error;}const byMall = new Map();for (const row of rows) {const mallId = String(row.mall_id || "").trim();if (!mallId) continue;if (!byMall.has(mallId)) {byMall.set(mallId, { violation_count: 0, inbound_exception_count: 0, high_count: 0, latest_date: null, by_type: {} });}const out = byMall.get(mallId);const cnt = Number(row.cnt) || 0;if (row.risk_type === "violation_goods") out.violation_count += cnt;if (row.risk_type === "inbound_exception") out.inbound_exception_count += cnt;if (String(row.severity || "").toLowerCase() === "high") out.high_count += cnt;if (row.latest_date && (!out.latest_date || String(row.latest_date) > String(out.latest_date))) out.latest_date = row.latest_date;out.by_type[row.risk_type] = (out.by_type[row.risk_type] || 0) + cnt;}return byMall;}async function querySettlementData(db, opts = {}) {const { startDate, endDate, attachCloudDb } = opts; // 1. fund_detail 按时间段
  let fundByMall;try {fundByMall = await buildFundDetailByMall(db, { startDate, endDate });} catch {fundByMall = new Map();}let fundSummaryByMall;try {fundSummaryByMall = await buildFundSummaryByMall(db, { startDate, endDate });} catch {fundSummaryByMall = new Map();}const riskByMall = new Map();let eprByMall;try {eprByMall = await buildEprFeeByMall(db);} catch {eprByMall = new Map();}let frozenByMall;try {frozenByMall = await buildFundFrozenByMall(db);} catch {frozenByMall = new Map();}let accountOverviewByMall;try {accountOverviewByMall = await buildAccountOverviewByMall(db);} catch {accountOverviewByMall = new Map();}let fulfillmentByMall;try {fulfillmentByMall = await buildFulfillmentBillByMall(db, { startDate, endDate });} catch {fulfillmentByMall = new Map();}let violationByMall;try {violationByMall = await buildViolationByMall(db, { startDate, endDate });} catch {violationByMall = new Map();} // 2. 销量/成本：纯本地数据（官方 salesv2 物化表 × erp_skus 加权均价）
  //    salesv2 是快照，只有 today/7d/30d 三个固定窗口，无法按自定义日期精确筛选。
  //    根据所选日期跨度选最接近的字段：<=7天用 last7d，>7天用 last30d。
  const finByMall = new Map();try {let daySpan = 30;if (startDate && endDate) {daySpan = Math.max(1, Math.round((new Date(endDate) - new Date(startDate)) / 86400000));}const salesCol = daySpan <= 7 ? "last7d_sales" : "last30d_sales";const rows = await queryAll(db, `
      SELECT s.mall_id,
       ...
       GROUP BY s.mall_id
    `);for (const r of rows) {finByMall.set(r.mall_id, { cost: Number(r.cost) || 0, qty: Number(r.qty) || 0 });}} catch {/* salesv2 表不存在时保持空 */}finByMall.clear(); // 2b. 结算订单口径（C 口径）：已结算订单明细 × 加权均价。
  ...
  const stlIncByMall = new Map();try {let siDateWhere, siParams;const siSourceSql = sourceWhere(db, "erp_temu_settlement_income");if (startDate && endDate) {siDateWhere = "AND stat_date >= ? AND stat_date <= ?";siParams = [startDate, endDate];} else {siDateWhere = "AND stat_date >= date('now', '-30 days')";siParams = [];}const siRows = await queryAll(db, `
      SELECT mall_id, SUM(income_amount) AS total_income, COUNT(*) AS days
       ...
       WHERE income_amount IS NOT NULL ${siDateWhere}
         ${siSourceSql}
       GROUP BY mall_id
    `, [...siParams]);for (const r of siRows) {stlIncByMall.set(r.mall_id, { income: Number(r.total_income) || 0, days: r.days || 0 });}} catch {/* settlement_income 表不存在时保持空 */} // 4. settlement_detail (全量不限时段)
  let sdByMall = null;try {sdByMall = await buildSettlementDetailByMall(db, { startDate, endDate });} catch {sdByMall = null;} // 5. 店铺字典
  const dict = await readMallDictionary(db);const dictByMall = new Map(dict.map((row) => [row.mall_id, row])); // 6. 合并所有出现过的 mall_id + 字典里所有 active 店铺（确保无数据的店也显示）
  const allMalls = new Set([...fundByMall.keys(), ...fundSummaryByMall.keys(), ...eprByMall.keys(), ...frozenByMall.keys(), ...accountOverviewByMall.keys(), ...fulfillmentByMall.keys(), ...violationByMall.keys(), ...stlIncByMall.keys(), ...orderByMall.keys()]);for (const row of dict) {if (row.status === "active") allMalls.add(row.mall_id);}const stores = [];for (const mallId of allMalls) {const d = dictByMall.get(mallId);if (!d || d.status === "test") continue;const fd = fundByMall.get(mallId) || null;const fs = fundSummaryByMall.get(mallId) || null;const risk = riskByMall.get(mallId) || null;const fin = finByMall.get(mallId) || null;const sd = sdByMall ? sdByMall.get(mallId) || null : null;const si = stlIncByMall.get(mallId) || null;stores.push({ mall_id: mallId, store_code: d.store_code || null, mall_name: d.mall_name || null, owner: d.owner || null, fund_detail: fd, fund_summary: fs, risk_detail: risk, epr_detail: eprByMall.get(mallId) || null, fund_frozen: frozenByMall.get(mallId) || null, account_overview: accountOverviewByMall.get(mallId) || null, fulfillment_bill: fulfillmentByMall.get(mallId) || null, violation: violationByMall.get(mallId) || null, settlement_income: si?.income || 0, // 真实结算收入（income-summary）
        settlement_income_days: si?.days || 0, cost: fin?.cost || 0, qty: fin?.qty || 0, settlement_detail: sd, order_detail: orderByMall.get(mallId) || null // C 口径：已结算订单件数/成本/金额
      });}stores.sort((a, b) => {if (!a.store_code && !b.store_code) return 0;if (!a.store_code) return 1;if (!b.store_code) return -1;return a.store_code.localeCompare(b.store_code);});return { stores, fund_detail_available: fundByMall.size > 0, fund_summary_available: fundSummaryByMall.size > 0, risk_detail_available: riskByMall.size > 0, epr_detail_available: eprByMall.size > 0, fund_frozen_available: frozenByMall.size > 0, account_overview_available: accountOverviewByMall.size > 0, fulfillment_bill_available: fulfillmentByMall.size > 0, violation_available: violationByMall.size > 0, financials_available: false, settlement_income_available: stlIncByMall.size > 0, order_detail_available: orderByMall.size > 0, date_range: { start: startDate || null, end: endDate || null } };}async function buildFinancialsByMall(db, attachCloudDb) {if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {return null;}let latest;try {latest = (await queryOne(db, "SELECT MAX(stat_date) AS d FROM cloud.temu_sales_snapshot"))?.d;} catch (error) {if (/no such table/i.test(String(error?.message || ""))) return null;throw error;}if (!latest) return new Map();const since = shiftDate(latest, -59); // 覆盖 last30d + prev30d 环比
  ...
  `, [since, latest]);const d7start = shiftDate(latest, -6);const d7prevStart = shiftDate(latest, -13);const d7prevEnd = shiftDate(latest, -7);const d30start = shiftDate(latest, -29);const d30prevStart = shiftDate(latest, -59);const d30prevEnd = shiftDate(latest, -30);const byMall = new Map();const ensure = (mallId) => {if (!byMall.has(mallId)) {const f = emptyFinancials();f.latest_date = latest;f._sku_rows = 0;f._cost_rows = 0;f._trend = new Map();byMall.set(mallId, f);}return byMall.get(mallId);};for (const row of rows) {const f = ensure(row.mall_id);const rev = Number(row.revenue) || 0;const cost = Number(row.cost) || 0;const qty = Number(row.qty) || 0;const gp = rev - cost;const d = row.d;f._trend.set(d, { date: d, revenue: rev, gross_profit: gp });if (d === latest) {f.today.revenue += rev;f.today.cost += cost;f.today.gross_profit += gp;f.today.qty += qty;}if (d >= d7start && d <= latest) {f.last7d.revenue += rev;f.last7d.cost += cost;f.last7d.gross_profit += gp;f.last7d.qty += qty;f._sku_rows += Number(row.sku_rows) || 0;f._cost_rows += Number(row.cost_rows) || 0;}if (d >= d7prevStart && d <= d7prevEnd) {f.last7d.revenue_prev += rev;}if (d >= d30start && d <= latest) {f.last30d.revenue += rev;f.last30d.cost += cost;f.last30d.gross_profit += gp;f.last30d.qty += qty;}if (d >= d30prevStart && d <= d30prevEnd) {f.last30d.revenue_prev += rev;}} // 收尾：算环比、成本覆盖率、趋势数组
  for (const f of byMall.values()) {f.last7d.rev_wow = f.last7d.revenue_prev > 0 ? (f.last7d.revenue - f.last7d.revenue_prev) / f.last7d.revenue_prev : null;f.last30d.rev_mom = f.last30d.revenue_prev > 0 ? (f.last30d.revenue - f.last30d.revenue_prev) / f.last30d.revenue_prev : null;f.cost_coverage = f._sku_rows > 0 ? f._cost_rows / f._sku_rows : null;f.trend_daily = Array.from(f._trend.values()).sort((a, b) => a.date.localeCompare(b.date));delete f._trend;delete f._sku_rows;delete f._cost_rows;}return byMall;} // 结算口径财务（实际结算明细 erp_temu_settlement_order_detail，纯本地、不依赖 cloud attach）。
...
async function buildSettlementProfitByMall(db) {const hasTable = await tableExists(db, "erp_temu_settlement_order_detail");if (!hasTable) return null;const latest = (await queryOne(db, "SELECT MAX(date(create_time)) AS d FROM erp_temu_settlement_order_detail"))?.d;if (!latest) return new Map();const since = shiftDate(latest, -59); // 覆盖 last30d + prev30d 环比
  const rows = await queryAll(db, `
  ...
  `, [since, latest]);const d7start = shiftDate(latest, -6);const d7prevStart = shiftDate(latest, -13);const d7prevEnd = shiftDate(latest, -7);const d30start = shiftDate(latest, -29);const d30prevStart = shiftDate(latest, -59);const d30prevEnd = shiftDate(latest, -30);const byMall = new Map();const ensure = (mallId) => {if (!byMall.has(mallId)) {const f = emptyFinancials();f.latest_date = latest;f._sku_rows = 0;f._cost_rows = 0;f._trend = new Map();byMall.set(mallId, f);}return byMall.get(mallId);};for (const row of rows) {const f = ensure(row.mall_id);const rev = Number(row.revenue) || 0;const cost = Number(row.cost) || 0;const qty = Number(row.qty) || 0;const gp = rev - cost;const d = row.d;f._trend.set(d, { date: d, revenue: rev, gross_profit: gp });if (d === latest) {f.today.revenue += rev;f.today.cost += cost;f.today.gross_profit += gp;f.today.qty += qty;}if (d >= d7start && d <= latest) {f.last7d.revenue += rev;f.last7d.cost += cost;f.last7d.gross_profit += gp;f.last7d.qty += qty;f._sku_rows += Number(row.sku_rows) || 0;f._cost_rows += Number(row.cost_rows) || 0;}if (d >= d7prevStart && d <= d7prevEnd) {f.last7d.revenue_prev += rev;}if (d >= d30start && d <= latest) {f.last30d.revenue += rev;f.last30d.cost += cost;f.last30d.gross_profit += gp;f.last30d.qty += qty;}if (d >= d30prevStart && d <= d30prevEnd) {f.last30d.revenue_prev += rev;}} // 收尾：算环比、成本覆盖率、趋势数组（与 buildFinancialsByMall 同口径）
  for (const f of byMall.values()) {f.last7d.rev_wow = f.last7d.revenue_prev > 0 ? (f.last7d.revenue - f.last7d.revenue_prev) / f.last7d.revenue_prev : null;f.last30d.rev_mom = f.last30d.revenue_prev > 0 ? (f.last30d.revenue - f.last30d.revenue_prev) / f.last30d.revenue_prev : null;f.cost_coverage = f._sku_rows > 0 ? f._cost_rows / f._sku_rows : null;f.trend_daily = Array.from(f._trend.values()).sort((a, b) => a.date.localeCompare(b.date));delete f._trend;delete f._sku_rows;delete f._cost_rows;}return byMall;} // 本地 ATTACH cloud 直接聚合 by-store（复刻 cloud dashboard.js /report/by-store）。
// 前提：db 已 ATTACH cloud。绕开 cloud HTTP 单进程瓶颈（by-store 重聚合会卡死 cloud event loop）。
async function buildByStoreLocal(db, options = {}) {const tid = options.tenantId || DEFAULT_CLOUD_TENANT;const includeTest = !!options.includeTest;const excludeMallIds = includeTest ? [] : ["MALL-EXT-E2E", "MALL-DBG"];const ph = excludeMallIds.map(() => "?").join(",");const baseMallFilter = excludeMallIds.length ? `AND mall_id NOT IN (${ph})` : "";const salesMallFilter = excludeMallIds.length ? `AND mall_supplier_id NOT IN (${ph})` : "";const cloudMalls = await optionalAllLocal(db, `SELECT mall_id, MAX(mall_name) AS mall_name, MAX(site) AS site, MAX(last_seen) AS last_seen_at
       FROM cloud.mall_accounts
      WHERE tenant_id = ? AND mall_id <> '' ${baseMallFilter}
      GROUP BY mall_id`, [tid, ...excludeMallIds]);const localMalls = await optionalAllLocal(db, `SELECT mall_id, mall_name, site, NULL AS last_seen_at
       FROM erp_temu_malls
      WHERE mall_id <> ''
        ${includeTest ? "" : "AND COALESCE(status,'active') <> 'test'"}`, []);const mallById = new Map();for (const m of cloudMalls) mallById.set(m.mall_id, m);for (const m of localMalls) {if (!mallById.has(m.mall_id)) mallById.set(m.mall_id, m);}const malls = Array.from(mallById.values()); // today_sales/last7d_sales/last30d_sales 是 Temu 在每个 stat_date 当天算好的快照值，
  // 表里每天每 SKU 一行（保留历史），所以只能取每店最新 stat_date 当天，绝不可跨天 SUM（否则虚高 N 倍）。
  const salesRows = await optionalAllLocal(db, `WITH latest AS (
     ...
     GROUP BY s.mall_supplier_id`, [tid, ...excludeMallIds, tid]);const salesMap = new Map(salesRows.map((r) => [r.mall_id, r])); // Keep report sales in step with OperationsWorkbench: prefer the official
  // salesv2 materialized table, and fall back to cloud scrape snapshots.
  const officialSalesRows = await optionalAllLocal(db, `SELECT s.mall_id,
            SUM(COALESCE(s.today_sales,0))   AS sales_today_qty,
            SUM(COALESCE(s.last7d_sales,0))  AS sales_7d_qty,
            SUM(COALESCE(s.last30d_sales,0)) AS sales_30d_qty,
      ...
      GROUP BY s.mall_id`, []);for (const r of officialSalesRows) salesMap.set(r.mall_id, r); // 待发口径：未发完(delivered<demand) 且排除终态。temu_status 是裸数字码、按 source_type 混 3 套枚举，
  ...
  const stockRows = await optionalAllLocal(db, `SELECT mall_id, COUNT(*) AS total_orders,
      ...
      WHERE tenant_id = ? AND mall_id <> '' ${baseMallFilter}
      GROUP BY mall_id`, [tid, ...excludeMallIds]);const stockMap = new Map(stockRows.map((r) => [r.mall_id, r])); // 货值（统一按官方 API 物化表 erp_temu_openapi_sku_sales 统计，本地主库表、35 店全覆盖，不依赖 cloud attach/抓包快照）：
  ...
  const invValueRows = await optionalAllLocal(db, `SELECT s.mall_id,
      ...
      GROUP BY s.mall_id`, []);const invValueMap = new Map(invValueRows.map((r) => [r.mall_id, r]));const activityRows = await optionalAllLocal(db, `SELECT mall_id, COUNT(*) AS activity_count,
      ...
      WHERE tenant_id = ? AND mall_id <> '' ${baseMallFilter}
      GROUP BY mall_id`, [tid, ...excludeMallIds]);const activityMap = new Map(activityRows.map((r) => [r.mall_id, r]));const shopStatsRows = await optionalAllLocal(db, `WITH latest AS (
     ...
     SELECT s.mall_id, s.stat_date, s.sale_volume, s.seven_days_sale_volume, s.thirty_days_sale_volume,
      ...
      WHERE s.tenant_id = ?`, [tid, ...excludeMallIds, tid]);const shopStatsMap = new Map(shopStatsRows.map((r) => [r.mall_id, r]));const officialShopStatsRows = await optionalAllLocal(db, `SELECT s.mall_id,
      ...
      WHERE s.mall_id <> ''
        ${includeTest ? "" : "AND COALESCE(m.status,'active') <> 'test'"}
      GROUP BY s.mall_id`, []);for (const r of officialShopStatsRows) shopStatsMap.set(r.mall_id, r);const healthRows = await optionalAllLocal(db, `SELECT mall_id, MAX(received_at) AS last_capture_at, COUNT(*) AS captures_total
       FROM cloud.capture_events
      WHERE tenant_id = ? AND mall_id IS NOT NULL AND mall_id <> '' ${baseMallFilter}
      GROUP BY mall_id`, [tid, ...excludeMallIds]);const healthMap = new Map(healthRows.map((r) => [r.mall_id, r]));const afterSalesRows = await optionalAllLocal(db, `SELECT mall_id, COUNT(*) AS after_sales_count
       FROM cloud.temu_after_sale_snapshot
      WHERE tenant_id = ? AND mall_id <> '' ${baseMallFilter}
      GROUP BY mall_id`, [tid, ...excludeMallIds]);const afterSalesMap = new Map(afterSalesRows.map((r) => [r.mall_id, r]));const now = Date.now();const stores = malls.map((m) => {const s = salesMap.get(m.mall_id) || {};const o = stockMap.get(m.mall_id) || {};const iv = invValueMap.get(m.mall_id) || {};const a = activityMap.get(m.mall_id) || {};const ss = shopStatsMap.get(m.mall_id) || {};const h = healthMap.get(m.mall_id) || {};const af = afterSalesMap.get(m.mall_id) || {};const lastCaptureAt = Number(h.last_capture_at || 0);return { mall_id: m.mall_id, mall_name: m.mall_name || null, site: m.site || null, mall_last_seen: m.last_seen_at || null, sales: { today_qty: toNum(s.sales_today_qty), last7d_qty: toNum(s.sales_7d_qty), last30d_qty: toNum(s.sales_30d_qty), sku_count: toNum(s.sku_count) }, stock_orders: { total: toNum(o.total_orders), pending: toNum(o.pending_orders), demand_qty: toNum(o.demand_qty_total), delivered_qty: toNum(o.delivered_qty_total) }, // 货值（官方 API 物化表统计，按加权均价计；成本未覆盖的 SKU 计 0，故为下限值）
        inventory: { warehouse_value: toNum(iv.warehouse_value), // 仓内货值：官方可售库存(warehouse_stock) × 加权均价
          in_transit_value: toNum(iv.in_transit_value) // 在途货值：官方待入库/在途(wait_in_stock) × 加权均价
        }, activities: { count: toNum(a.activity_count), unique: toNum(a.unique_activities), skc_count: toNum(a.activity_skc_count) }, shop_stats: { stat_date: ss.stat_date || null, sale_volume: toNum(ss.sale_volume), sale_7d: toNum(ss.seven_days_sale_volume), sale_30d: toNum(ss.thirty_days_sale_volume), on_sale_skc: toNum(ss.on_sale_product_number), wait_skc: toNum(ss.wait_product_number), lack_skc: toNum(ss.lack_skc_number), advice_prepare_skc: toNum(ss.advice_prepare_skc_number), about_to_sell_out_skc: toNum(ss.about_to_sell_out_number), already_sold_out_skc: toNum(ss.already_sold_out_number), high_price_limit_skc: toNum(ss.high_price_limit_number), after_sale_ratio_90d: ss.quality_after_sale_ratio_90d ?? null, last_updated_at: ss.last_updated_at || null }, after_sales: { count: toNum(af.after_sales_count) }, health: { last_capture_at: lastCaptureAt || null, captures_total: toNum(h.captures_total), lag_seconds: lastCaptureAt ? Math.max(0, Math.round((now - lastCaptureAt) / 1000)) : null } };}); // 销量数据实际覆盖窗口（cloud 回溯天数可能不足 30 天，前端据此标注真实天数，避免"近30天"误导）
  const win = await optionalAllLocal(db, `SELECT MIN(stat_date) mn, MAX(stat_date) mx, COUNT(DISTINCT stat_date) days
       FROM cloud.temu_sales_snapshot WHERE tenant_id = ? AND mall_supplier_id <> ''`, [tid])[0] || {};return { generated_at: now, tenant_id: tid, store_count: stores.length, stores, sales_window: { start: win.mn || null, end: win.mx || null, days: toNum(win.days) } };} // 建议备货自算（统一口径，替代 Temu 黑盒 adviceQuantity）——与 temuAutoPurchase.cjs calcAdvice 同公式。
const _RESTOCK_FAST_QTY = 50,_RESTOCK_DAYS_NORMAL = 10,_RESTOCK_DAYS_FAST = 7;function _beijingHour() {return (new Date().getUTCHours() + 8) % 24;}function _skuTotalStock(warehouse, unavail, lack, waitIn) {return (warehouse || 0) + (unavail || 0) - (lack || 0) + (waitIn || 0);}function _calcAdvice(today, last7d, totalStock) {const hour = _beijingHour();const daily = Math.max((last7d || 0) / 7, (today || 0) * (hour < 12 ? 2 : hour < 18 ? 1.5 : 1.3));const days = daily > _RESTOCK_FAST_QTY ? _RESTOCK_DAYS_FAST : _RESTOCK_DAYS_NORMAL;return Math.max(0, Math.ceil(daily * days - (totalStock || 0)));} // 进程级结果缓存：跨库聚合冷态可达 ~17s（OS page cache 被挤出后首查），
// 缓存 + 服务端定时预热让用户请求永远命中暖缓存、秒回。
const REPORT_CACHE_TTL_MS = 15 * 60 * 1000; // 读运营报表物化缓存(erp_report_cache,由 refresh-ops-reports cron 预聚合)。
...
async function _readOpsReportCache(db, cacheKey) {try {const row = await queryOne(db, "SELECT payload_json, updated_at FROM erp_report_cache WHERE cache_key = ?", [cacheKey]);if (row && row.payload_json) {const d = JSON.parse(row.payload_json);d.cached_at = row.updated_at || null;return d;}} catch (e) {/* 表不存在/解析失败 → 走实时兜底 */}return null;}const _reportCache = new Map(); // includeTest -> { data, ts }
const _reportInflight = new Map(); // includeTest -> Promise（并发去重，避免多请求同时冷算）
function clearMultiStoreReportCache() {_reportCache.clear();_reportInflight.clear();} // 主入口（带缓存）：force=true 跳过读缓存（预热用），仍写缓存。
async function buildMultiStoreReport(db, options = {}) {if (!db) throw new Error("multiStoreReport: db is required (host mode only)");const key = options.includeTest ? "1" : "0";if (!options.force) {const cached = _reportCache.get(key);if (cached && Date.now() - cached.ts < REPORT_CACHE_TTL_MS) return cached.data;}const inflight = _reportInflight.get(key);if (inflight) return inflight; // 复用进行中的计算（含预热），避免并发重复冷算
  const p = Promise.resolve().then(async () => await _buildMultiStoreReportFresh(db, options)).then((data) => {_reportCache.set(key, { data, ts: Date.now() });return data;}).finally(() => {_reportInflight.delete(key);});_reportInflight.set(key, p);return p;} // 物化优先版：先读 erp_report_cache 的 multi_store:<test> 物化结果（毫秒），
// miss/无表时回退实时 buildMultiStoreReport（冷态 ~46s 独占单进程，故须靠 cron 喂表）。
// async：缓存命中也走 async 签名，与既有调用点 await 一致。
async function getMultiStoreReportFast(db, options = {}) {const mat = await _readOpsReportCache(db, "multi_store:" + (options.includeTest ? "1" : "0"));if (mat) return mat;return await buildMultiStoreReport(db, options);} // 预热：强制重算并填缓存，供服务端定时调用，使 page cache 常暖、用户不撞冷查询。
async function prewarmMultiStoreReport(db, attachCloudDb) {try {await buildSkuSales(db, { includeTest: false, attachCloudDb, force: true });} catch {}return await buildMultiStoreReport(db, { includeTest: false, attachCloudDb, force: true }).catch(() => null);} // 真正的聚合实现（无缓存）
async function _buildMultiStoreReportFresh(db, options = {}) {if (!db) throw new Error("multiStoreReport: db is required (host mode only)");const attachCloudDb = options.attachCloudDb;const attached = typeof attachCloudDb === "function" && attachCloudDb(db) === true; // 优先本地 ATTACH cloud 直接聚合（快、不触碰 cloud HTTP 单进程）；attach 不可用时退回 HTTP API
  let cloud; // 财务口径：实际结算明细（erp_temu_settlement_order_detail），纯本地、不依赖 cloud attach。
  // 货号 99.96% 齐全、成本匹配 ~97.6%，替代原销量×申报价预估口径（buildFinancialsByMall，
  // 因 cloud 销量快照货号缺失把无成本 SKU 按 0 成本算、致部分店毛利虚高）。
  let financialsByMall = null;try {financialsByMall = await buildSettlementProfitByMall(db);} catch (error) {financialsByMall = null;if (options.onError) options.onError(error);}if (attached) {cloud = await buildByStoreLocal(db, { includeTest: options.includeTest });} else {// 降级：本地开发机无 cloud sqlite 等场景，退回 cloud HTTP API（by-store 维度不可用）
    cloud = await fetchCloudReport({ includeTest: options.includeTest });} // 实际结算收入（本地物化表 erp_temu_settlement_income，纯本地、不依赖 cloud attach，
  // 与销量×申报价预估营收 financials 独立旁路）。表缺失/无数据时返回 null，前端据此知道维度不可用。
  let settlementByMall = null;try {settlementByMall = await buildSettlementIncomeByMall(db);} catch (error) {settlementByMall = null;if (options.onError) options.onError(error);} // 结算明细三态（待处理/结算中/已到账，物化表 erp_temu_settlement_detail），与收入汇总独立旁路。
  let settlementDetailByMall = null;try {settlementDetailByMall = await buildSettlementDetailByMall(db);} catch (error) {settlementDetailByMall = null;if (options.onError) options.onError(error);} // 对账中心账务明细（fund_detail），按店铺聚合近 30 天费用分类汇总
  let fundDetailByMall = null;try {fundDetailByMall = await buildFundDetailByMall(db);} catch (error) {fundDetailByMall = null;if (options.onError) options.onError(error);}let fundSummaryByMall = null;try {fundSummaryByMall = await buildFundSummaryByMall(db);} catch (error) {fundSummaryByMall = null;if (options.onError) options.onError(error);}let settlementRiskByMall = null;try {settlementRiskByMall = attached ? await buildSettlementRiskByMall(db) : null;} catch (error) {settlementRiskByMall = null;if (options.onError) options.onError(error);}const dict = await readMallDictionary(db);const dictByMall = new Map(dict.map((row) => [row.mall_id, row]));const stores = [];const unmapped = [];for (const s of cloud.stores || []) {const dictRow = dictByMall.get(s.mall_id);const financials = financialsByMall ? financialsByMall.get(s.mall_id) || emptyFinancials() : null;const settlement = settlementByMall ? settlementByMall.get(s.mall_id) || emptySettlement() : null;const settlement_detail = settlementDetailByMall ? settlementDetailByMall.get(s.mall_id) || emptySettlementDetail() : null;const fund_detail = fundDetailByMall ? fundDetailByMall.get(s.mall_id) || null : null;const fund_summary = fundSummaryByMall ? fundSummaryByMall.get(s.mall_id) || null : null;const risk_detail = settlementRiskByMall ? settlementRiskByMall.get(s.mall_id) || null : null;const enriched = { ...s, store_code: dictRow?.store_code || null, store_status: dictRow?.status || "unknown", dict_remark: dictRow?.remark || null, owner: dictRow?.owner || null, financials, settlement, settlement_detail, fund_detail, fund_summary, risk_detail };if (!dictRow) {unmapped.push(enriched);} else if (options.includeTest || dictRow.status !== "test") {stores.push(enriched);}} // 按 store_code 排序，无 store_code 排最后
  stores.sort((a, b) => {if (!a.store_code && !b.store_code) return 0;if (!a.store_code) return 1;if (!b.store_code) return -1;return a.store_code.localeCompare(b.store_code);});return { generated_at: cloud.generated_at || Date.now(), cloud_tenant_id: cloud.tenant_id || null, store_count: stores.length, financials_available: financialsByMall !== null, settlement_available: settlementByMall !== null, // 实际结算收入维度是否有数据（抓包物化）
    ...
    fund_summary_available: fundSummaryByMall !== null && fundSummaryByMall.size > 0, risk_detail_available: settlementRiskByMall !== null && settlementRiskByMall.size > 0, sales_window: cloud.sales_window || null, // 销量数据实际覆盖窗口（不足30天时前端据此标注真实天数）
    stores, unmapped // 云端有数据但本地字典没登记的（应该提醒用户去 seed）
  };}
// ===== 销售管理：SKU 级明细（每店最新天，含动销/库存/申报价/售罄）=====
const _skuSalesCache = new Map(); // includeTest -> { data, ts }

async function _buildSkuSalesFresh(db, options = {}) {
  const tid = options.tenantId || DEFAULT_CLOUD_TENANT;
  const includeTest = !!options.includeTest;
  const limit = Math.min(8000, Math.max(50, Number(options.limit) || 4000));
  const rows = await optionalAllLocal(db, `
    WITH latest AS (
      SELECT mall_supplier_id, MAX(stat_date) AS sd
        FROM cloud.temu_sales_snapshot
...
async function _buildSkuSalesOfficialFresh(db, options = {}) {
  const includeTest = !!options.includeTest;
  const limit = Math.min(8000, Math.max(50, Number(options.limit) || 4000));
  const rows = await optionalAllLocal(db, `
...
async function buildRiskList(db, options = {}) {
  ...
  if (!options.force) {
    ...
    const mat = await _readOpsReportCache(db, "risk_list:" + key);
...
async function buildActivityList(db, options = {}) {
  ...
  if (!options.force) {
    ...
    if (c && Date.now() - c.ts < REPORT_CACHE_TTL_MS) return c.data;
    const mat = await _readOpsReportCache(db, "activity_list:" + key);
  ...
  const rows = await optionalAllLocal(db, `
    ...
    SELECT a.mall_id, m.store_code, m.mall_name, a.activity_kind, a.activity_title, a.activity_status,
           ...
           a.activity_id, a.product_id, a.activity_type, a.sku_id, a.goods_id, a.site,
  ...
  const out = rows.map((a) => ({
    ...
    activity_id: a.activity_id || null, product_id: a.product_id || null, goods_id: a.goods_id || null, spu_id: a.spu_id || null, site: a.site || null,
  ...
  const enrolledRows = await optionalAllLocal(db, `
    ...
    SELECT e.mall_id, e.sku_ext_code, e.product_id, COUNT(DISTINCT e.activity_thematic_id) AS enrolled_count
     ...
     GROUP BY e.mall_id, e.sku_ext_code, e.product_id`, [tid, tid]);
  ...
  const enrollDetails = await optionalAllLocal(db, `
  ...
  const crossRows = await optionalAllLocal(db, `
  ...
  for (const e of pmap.values()) {
    ...
    for (const a of e.activities) {
      ...
      const ref = a.suggested_price != null ? a.suggested_price : a.signup_price;
...
async function _buildShopHealthScrapeFresh(db, options = {}) {
  const tid = options.tenantId || DEFAULT_CLOUD_TENANT;
  const rows = await optionalAllLocal(db, `
    WITH latest AS (
...
async function _buildShopHealthOfficialFresh(db, options = {}) {
  const rows = await optionalAllLocal(db, `
    SELECT s.mall_id, m.store_code, m.mall_name, m.owner,
           SUM(COALESCE(s.today_sales,0))   AS sale_volume,
...
async function buildGoodsDataSnapshot(db, options = {}) {
...
async function _buildStockOrdersOfficialFresh(db, options = {}) {
  const includeTest = !!options.includeTest;
  const where = includeTest ? "" : " AND COALESCE(m.status,'active') <> 'test'";
  // 1) 发货单：按 mall_id+SKC 汇总在途件数(inboundTime 为空=未入库；在途 = 已发件数 - 已收件数)
  const shipRows = await optionalAllLocal(db, "SELECT r.mall_id, r.raw_json FROM erp_temu_openapi_records r LEFT JOIN erp_temu_malls m ON m.mall_id = r.mall_id WHERE r.source = 'ship_order' AND r.raw_json IS NOT NULL" + where, []);
  ...
  const poRows = await optionalAllLocal(db, "SELECT r.mall_id, m.store_code, m.mall_name, r.raw_json FROM erp_temu_openapi_records r LEFT JOIN erp_temu_malls m ON m.mall_id = r.mall_id WHERE r.source = 'purchase_order' AND r.raw_json IS NOT NULL" + where, []);
  const out = [];
...
async function buildStockOrders(db, options = {}) {
...
async function buildSalesTrend(db, options = {}) {
...
async function buildProductPanel(db, options = {}) {
  ...
  const flowRows = await optionalAllLocal(db, `
  ...
  const limRows = await optionalAllLocal(db, `
  ...
  const actRows = await optionalAllLocal(db, `
  ...
  const compRows = await optionalAllLocal(db, `
  ...
  const malls = await optionalAllLocal(db, `SELECT mall_id, store_code, mall_name, status FROM erp_temu_malls`, []);
  ...
  if (pids.length) {
    ...
    const titleRows = await optionalAllLocal(db, `
    ...
    const detailRows = await optionalAllLocal(db, `
  ...
  if (pids.length) {
    ...
    const shipRows = await optionalAllLocal(db, `
...
async function _buildProductPanelOfficialFresh(db, options = {}) {
  const includeTest = !!options.includeTest;
  const limit = Math.min(8000, Math.max(50, Number(options.limit) || 4000));
  const rows = await optionalAllLocal(db, `
...
async function buildProductSalesTrend(db, options = {}) {
...
async function buildHighPriceFlowList(db, options = {}) {
  ...
  if (!options.force) {
    ...
    if (c && Date.now() - c.ts < REPORT_CACHE_TTL_MS) return c.data;
    const mat = await _readOpsReportCache(db, "high_price_flow:" + key);
    ...
    WITH lr AS (
      SELECT mall_id, product_id, MAX(stat_date) sd
       ...
       WHERE tenant_id = ? AND risk_type = 'high_price_flow'
         ...
         AND stat_date >= ?
       GROUP BY mall_id, product_id)
    SELECT r.mall_id, r.product_id, MAX(lr.sd) AS last_seen_date,
           MAX(r.skc_id) skc_id,
     ...
     GROUP BY r.mall_id, r.product_id`, [tid, fromDate]);
  if (!limRows.length) {
  ...
  const offRows = await optionalAllLocal(db, `
  ...
  const cloudRows = await optionalAllLocal(db, `
    WITH ls AS (SELECT product_id, MAX(stat_date) sd FROM cloud.temu_sales_snapshot WHERE tenant_id = ? AND product_id IN (${ph}) GROUP BY product_id)
  ...
  // 4) 店铺信息(店号/店名/负责人)
  const malls = await optionalAllLocal(db, `SELECT mall_id, store_code, mall_name, owner, status FROM erp_temu_malls`, []);
  const mallMap = new Map(malls.map((m) => [m.mall_id, m]));
  const out = [];
...
// 高价限流详情：返回某商品的完整 SKU 级限流数据（规格/原价/目标价/折扣/竞品/限流站点）
async function getHighPriceFlowDetail(db, options = {}) {
  if (!db) throw new Error("getHighPriceFlowDetail: db is required");
  const { mallId, productId, attachCloudDb } = options;
  if (!mallId || !productId) return null;
  if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) return null;
  const tid = options.tenantId || DEFAULT_CLOUD_TENANT;
  const rows = await optionalAllLocal(db, `
    SELECT raw_json, stat_date AS last_seen_date
      FROM cloud.temu_operation_risk_snapshot
     WHERE tenant_id = ? AND mall_id = ? AND risk_type = 'high_price_flow'
       AND product_id = ?
     ORDER BY stat_date DESC LIMIT 1`, [tid, mallId, String(productId)]);
  const row = rows[0];
  if (!row || !row.raw_json) return null;
  let raw;
  try {raw = JSON.parse(row.raw_json);} catch {return null;}
  const skcList = Array.isArray(raw.productSkcList) ? raw.productSkcList :
  Array.isArray(raw.skcInfoList) ? raw.skcInfoList : [];
  const skus = [];
  for (const skc of skcList) {
    ...
    for (const sku of skuList) {
      const specs = Array.isArray(sku.specList) ? sku.specList.map((s) => [s.parentSpecName, s.specName].filter(Boolean).join(": ")).join(" / ") : "";
      const cur = Number(sku.currentSupplierPrice) || 0;
      const tgt = Number(sku.targetSupplierPriceForAllSite) || 0;
      skus.push({
        sku_id: String(sku.productSkuId || ""),
        ext_code: sku.extCode || null,
        spec: specs,
        current_price: cur / 100,
        target_price: tgt / 100,
        discount: cur > 0 ? Number((tgt / cur * 10).toFixed(2)) : null,
        has_competitor: !!(sku.competitor && typeof sku.competitor === "object" && sku.competitor.competitorId),
        competitor_name: sku.competitor?.name || null,
        competitor_image: sku.competitor?.competitorImageUrl || null
      });
    }
  }
  const siteList = Array.isArray(raw.siteList) ? raw.siteList :
  Array.isArray(raw.flowReduceSiteList) ? raw.flowReduceSiteList :
  Array.isArray(raw.limitSiteList) ? raw.limitSiteList : [];
  const sitePriceMap = new Map();
  const productSiteIds = new Set(siteList.map((s) => s.siteId));
  try {
    let spRows = await optionalAllLocal(db, `
      SELECT raw_json FROM cloud.temu_operation_risk_snapshot
       WHERE tenant_id = ? AND mall_id = ? AND risk_type = 'high_price_flow'
         AND product_id = ? AND raw_json LIKE '%targetSupplierPrice%'
       ORDER BY stat_date DESC LIMIT 200`, [tid, mallId, String(productId)]);
    if (!spRows.length && productSiteIds.size > 0) {
      const allSp = await optionalAllLocal(db, `
        SELECT raw_json, source_event_id FROM cloud.temu_operation_risk_snapshot
         WHERE tenant_id = ? AND mall_id = ? AND risk_type = 'high_price_flow'
           AND product_id IS NULL AND raw_json LIKE '%targetSupplierPrice%'
         ORDER BY stat_date DESC LIMIT 500`, [tid, mallId]);
      const groups = new Map();
      for (const sp of allSp) {
        const eid = sp.source_event_id || "_";
        if (!groups.has(eid)) groups.set(eid, []);
        groups.get(eid).push(sp);
      }
      let bestRows = null,bestOverlap = 0;
      for (const [, rows] of groups) {
        let overlap = 0;
        for (const r of rows) {try {const s = JSON.parse(r.raw_json);if (productSiteIds.has(s.siteId)) overlap++;} catch {}}
        if (overlap > bestOverlap) {bestOverlap = overlap;bestRows = rows;}
      }
      if (bestRows && bestOverlap >= Math.min(productSiteIds.size, 3)) spRows = bestRows;
    }
    for (const sp of spRows) {
      try {
        const s = JSON.parse(sp.raw_json);
        if (s.siteId && s.targetSupplierPrice != null) sitePriceMap.set(s.siteId, Number(s.targetSupplierPrice) / 100);
      } catch {}
    }
  } catch {}
  // 没有每站价格数据时，用 SKU 全站目标价（targetSupplierPriceForAllSite）作为兜底
  let fallbackTargetPrice = null;
  if (sitePriceMap.size === 0 && skus.length > 0) {
    const tgtPrices = skus.map((s) => s.target_price).filter((v) => v > 0);
    if (tgtPrices.length > 0) fallbackTargetPrice = Math.min(...tgtPrices);
  }
  const sites = siteList.map((s) => ({ id: s.siteId, name: s.siteName, target_price: sitePriceMap.get(s.siteId) ?? fallbackTargetPrice }));
  const minTarget = skus.length ? Math.min(...skus.map((s) => s.target_price).filter((v) => v > 0)) : null;
  const maxDiscount = skus.length ? Math.max(...skus.map((s) => s.discount || 0)) : null;
  return {
    product_id: String(raw.productId || productId),
    product_name: raw.productName || raw.productTitle || null,
    image: raw.materialImageUrl || null,
    skc_id: skcList[0]?.productSkcId || null,
    flow_decline_rate: raw.flowDeclineRate ?? null,
    flow_reduce_tag: raw.flowReduceTagType,
    last_seen_date: row.last_seen_date,
    site_count: sites.length,
    sites,
    skus,
    min_target_price: minTarget,
    max_discount: maxDiscount
  };
}

// 商品运营面板:优先读物化缓存表(cron 预聚合,毫秒),无/未跑则实时兜底(慢);官方源走 official 版
async function getProductPanelFast(db, options = {}) {
  if (useOfficialReports(options)) {
    const ok = "o:" + (options.includeTest ? "1" : "0");
    if (!options.force) {const c = _productPanelCache.get(ok);if (c && Date.now() - c.ts < REPORT_CACHE_TTL_MS) return c.data;}
    const data = await _buildProductPanelOfficialFresh(db, options);
    _productPanelCache.set(ok, { data, ts: Date.now() });
    return data;
  }
  const mat = await _readOpsReportCache(db, "product_panel:" + (options.includeTest ? "1" : "0"));
  if (mat) return mat;
  return buildProductPanel(db, options);
}

// 写入店铺负责人（host 模式）。返回受影响行数。
async function setMallOwner(db, mallId, owner) {
...
async function setOpTaskState(db, taskKey, status, owner) {
  ...
  if (status == null) {
    return (await execute(db, "DELETE FROM op_task_state WHERE task_key = ?", [key])).changes;
  }
  ...
  key, status, normOwner, Date.now()])).changes;
}
...
async function buildPurchaseReport(db, options = {}) {
  ...
  const sr = await queryOne(db, `
    SELECT COUNT(*) po_count,
      ...
      FROM erp_purchase_orders`, [curMonth, curMonth]);
  const summary = {
  ...
  // 2) 状态分布(全量)
  const by_status = (await optionalAllLocal(db, `SELECT status, COUNT(*) count, SUM(${AMT}) amount FROM erp_purchase_orders GROUP BY status`, [])).
  ...
  const by_supplier = (await optionalAllLocal(db, `
  ...
  // 4) 月趋势(全量，排除取消，近12月)
  const monthly = (await optionalAllLocal(db, `
  ...
  const cap = async (w) => {const r = await queryOne(db, `SELECT COUNT(*) c, COALESCE(SUM(${AMT}),0) a FROM erp_purchase_orders WHERE ${w}`);return { count: toNum(r.c), amount: toNum(r.a) };};
  const capital = {
    paid_done: await cap(`${ACT} AND payment_status='paid' AND ${DONE}`),
    paid_undone: await cap(`${ACT} AND payment_status='paid' AND NOT (${DONE})`),
    unpaid_done: await cap(`${ACT} AND COALESCE(payment_status,'unpaid')<>'paid' AND ${DONE}`),
    unpaid_undone: await cap(`${ACT} AND COALESCE(payment_status,'unpaid')<>'paid' AND NOT (${DONE})`)
  };
  // 7) 应付账款账龄(未付单，按下单距今分桶；系统无账期/交期字段，只能按下单日近似)
  const agingRaw = await optionalAllLocal(db, `
  ...
  const cashMonthly = (await optionalAllLocal(db, `
    SELECT substr(paid_at,1,7) month, COUNT(*) count, COALESCE(SUM(${AMT}),0) amount
     ...
     GROUP BY month ORDER BY month DESC LIMIT 12`, [])).
  map((r) => ({ month: r.month, count: toNum(r.count), amount: toNum(r.amount) })).reverse();
  const covR = await queryOne(db, `SELECT SUM(CASE WHEN paid_at IS NOT NULL AND paid_at<>'' THEN 1 ELSE 0 END) f, COUNT(*) c FROM erp_purchase_orders WHERE ${ACT} AND payment_status='paid'`);
  ...
  const psT = await queryOne(db, `SELECT COUNT(*) c, COALESCE(SUM(${AMT}),0) a FROM erp_purchase_orders WHERE ${UNSHIP}`);
  const psAgingRaw = await optionalAllLocal(db, `
  ...
  if (ids.length) {
    ...
    const lineAgg = await optionalAllLocal(db, `SELECT po_id, COUNT(*) line_count, COALESCE(SUM(qty),0) total_qty, COALESCE(SUM(received_qty),0) received_qty FROM erp_purchase_order_lines WHERE po_id IN (${ph}) GROUP BY po_id`, ids);
...
async function fetchQcFlawImages(db, options = {}) {
  ...
  const m = await queryOne(db, "SELECT app_key, app_secret, access_token, region FROM erp_temu_openapi_auth WHERE mall_id = ?", [mallId]);
...
async function buildOpenapiQc(db, options = {}) {
  const includeTest = !!options.includeTest;
...
async function getOpenapiQcFast(db, options = {}) {
  const mat = await _readOpsReportCache(db, "openapi_qc:" + (options.includeTest ? "1" : "0"));
  if (mat) return mat;
...
async function buildFirstShipToday(db, options = {}) {
  const includeTest = !!options.includeTest;
...
async function buildGoodsCreatedToday(db, options = {}) {
  const includeTest = !!options.includeTest;
...
async function buildQualityPanel(db, options = {}) {
  const attachCloudDb = options.attachCloudDb;
  if (typeof attachCloudDb !== "function" || attachCloudDb(db) !== true) {
  ...
  const dict = new Map();
  for (const m of await readMallDictionary(db)) {
  ...
  const pageRows = await optionalAllLocal(db, `
  ...
  const shopRows = await optionalAllLocal(db, `
    SELECT mall_id, url, body_json, received_at
      FROM cloud.capture_events
     WHERE url_path = ? AND mall_id IS NOT NULL AND mall_id <> ''
     ORDER BY received_at ASC
...
async function getQualityPanelFast(db, options = {}) {
  const mat = await _readOpsReportCache(db, "quality_panel:" + (options.includeTest ? "1" : "0"));
  if (mat) return mat;
...
async function buildReviews(db, options = {}) {
  const includeTest = !!options.includeTest;
  ...
  try {
    ...
    for (const r of out) {
      if (r.score != null && r.score <= 2) {
        ...
        await execute(db, "INSERT INTO op_task_state (task_key, status, owner, note, updated_at) VALUES (?, 'pending', NULL, ?, ?) ON CONFLICT(task_key) DO NOTHING", [`review_alert|${r.mall_id}|${r.review_id}`, `差评(${r.score}分): ${snippet}`, Date.now()]);
      }
    }
...
async function buildWarehouseInventory(db, options = {}) {
  const rows = await optionalAllLocal(db,
  `SELECT
       b.account_id,
...
async function buildPipelineOverview(db, options = {}) {
  if (process.env.PIPELINE_USE_PANEL !== "0") return await buildPipelineOverviewFast(db, options);
...
async function buildPipelineOverviewFast(db, options = {}) {
  if (!db) throw new Error("buildPipelineOverviewFast: db is required");
  if (!options.force) {
    const c = _pipelineCache.get("pf");
    if (c && Date.now() - c.ts < REPORT_CACHE_TTL_MS) return c.data;
    const mat = await _readOpsReportCache(db, "pipeline_overview");
    if (mat) {_pipelineCache.set("pf", { data: mat, ts: Date.now() });return mat;}
  }
  const includeTest = !!options.includeTest;

  // 1) 主体:官方版商品面板(全店权威库存/销量)
  const base = (await _buildProductPanelOfficialFresh(db, { includeTest, force: true })).rows || [];
  ...
  if (cloudOk) {
    const tid = options.tenantId || DEFAULT_CLOUD_TENANT;
    for (const f of await optionalAllLocal(db, `
      WITH lf AS (SELECT mall_id, MAX(stat_date) sd FROM cloud.temu_product_flow_snapshot WHERE tenant_id = ? GROUP BY mall_id)
    ...
    for (const r of await optionalAllLocal(db, `
      WITH lr AS (SELECT mall_id, MAX(stat_date) sd FROM cloud.temu_operation_risk_snapshot WHERE tenant_id = ? AND risk_type = 'high_price_flow' GROUP BY mall_id)
      SELECT DISTINCT r.mall_id, r.product_id FROM cloud.temu_operation_risk_snapshot r JOIN lr ON lr.mall_id = r.mall_id AND lr.sd = r.stat_date
       WHERE r.risk_type = 'high_price_flow' AND r.product_id IS NOT NULL AND r.product_id <> ''`, [tid])) {
      limSet.add(r.mall_id + "|" + r.product_id);
    }
    for (const a of await optionalAllLocal(db, `
      WITH la AS (SELECT mall_id, MAX(stat_date) sd FROM cloud.temu_activity_snapshot WHERE tenant_id = ? GROUP BY mall_id)
      SELECT a.mall_id, a.product_id, COUNT(*) cnt, MIN(a.signup_price_cents) minp
        FROM cloud.temu_activity_snapshot a JOIN la ON la.mall_id = a.mall_id AND la.sd = a.stat_date
       WHERE a.product_id IS NOT NULL AND a.product_id <> '' AND a.signup_price_cents IS NOT NULL
    ...
    for (const c of await optionalAllLocal(db, `
    ...
    for (const v of await optionalAllLocal(db, `
    ...
    for (const ev of await optionalAllLocal(db, `
      SELECT mall_id, url, body_json, received_at
        FROM cloud.capture_events
       WHERE url_path = ? AND mall_id IS NOT NULL AND mall_id <> ''
    ...
    for (const s of await optionalAllLocal(db, `
      WITH ls AS (SELECT mall_supplier_id, MAX(stat_date) sd FROM cloud.temu_sales_snapshot WHERE tenant_id = ? GROUP BY mall_supplier_id)
      SELECT s.mall_supplier_id AS mall_id, s.product_id, MIN(NULLIF(s.declared_price_cents,0)) dp, MAX(s.asf_score) score, MAX(s.comment_num) comments
        FROM cloud.temu_sales_snapshot s JOIN ls ON ls.mall_supplier_id = s.mall_supplier_id AND ls.sd = s.stat_date
       WHERE s.product_id IS NOT NULL AND s.product_id <> '' GROUP BY s.mall_supplier_id, s.product_id`, [tid])) {
  ...
  const purchasingPids = new Set((await optionalAllLocal(db, `
  ...
  map((r) => String(r.pid)));
  const inboundPids = new Set((await optionalAllLocal(db, `
  ...
  try {await execute(db, "INSERT INTO erp_report_cache (cache_key, payload_json, updated_at) VALUES (?, ?, datetime('now')) ON CONFLICT(cache_key) DO UPDATE SET payload_json=excluded.payload_json, updated_at=excluded.updated_at", ["pipeline_overview", JSON.stringify(data)]);} catch {/* 表不存在时静默 */}
  return data;
}
...
function _computePipelineRiskTags(o) {
  const tags = [];
  if (o.lack_qty > 0 && o.advice_qty > 0 && o.warehouse_stock <= 0) tags.push("urgent_restock");else
  if (o.lack_qty > 0) tags.push("stock_out");
...
async function _buildPipelineOverviewLegacy(db, options = {}) {
  ...
  for (const r of await optionalAllLocal(db, `SELECT sku_id, SUM(available_qty) aq, SUM(reserved_qty) rq FROM erp_inventory_batches GROUP BY sku_id`, [])) {
  ...
  // 7) 遍历所有活跃 SKU，分配阶段
  const skus = await optionalAllLocal(db, `
  ...
  for (const sku of skus) {
    ...
    let stage;
    if (s && s.m30 > 0 && s.adv > 0) stage = "needs_restock";else
    if (s && s.m30 > 0) stage = "selling";else
    if (inv) stage = "in_stock";else
    if (inboundSkus.has(sku.id)) stage = "inbound";else
    if (purchasingSkus.has(sku.id)) stage = "purchasing";else
    stage = "created";
...
// 单品风险标签查询（给商品面板用）
async function buildProductRiskTags(db, options = {}) {
  ...
  const salesRows = await optionalAllLocal(db, `
  ...
  const skuRows = await optionalAllLocal(db, `SELECT id, internal_sku_code, temu_skc_id FROM erp_skus WHERE internal_sku_code IN (${ph})`, skuCodes);
  const skcIds = skuRows.map((r) => r.temu_skc_id).filter(Boolean);

  let revMap = new Map();
  if (skcIds.length) {
    const ph2 = skcIds.map(() => "?").join(",");
    const revRows = await optionalAllLocal(db, `
  ...
  const retRows = await optionalAllLocal(db, `SELECT sku_code, COUNT(*) cnt FROM consign_after_sale_items WHERE sku_code IN (${ph}) GROUP BY sku_code`, skuCodes);
...
async function buildFlowAnalysis(db, options = {}) {
  ...
  const tid = options.tenantId || DEFAULT_CLOUD_TENANT;
  const available_dates = (await optionalAllLocal(db, `
  ...
  const flowRows = effectiveDate ? await optionalAllLocal(db, `
    SELECT ${selectCols}
  ...
  const malls = await optionalAllLocal(db, `SELECT mall_id, store_code, mall_name FROM erp_temu_malls`, []);
  const mallMap = new Map(malls.map((m) => [m.mall_id, m]));
...
async function buildFlowTrend(db, options = {}) {
  ...
  const snapshotSql = `SELECT stat_date, expose_num, click_num, detail_visitor_num,
     ...
     WHERE tenant_id = ? AND mall_id = ? AND product_id = ?
           AND COALESCE(goods_id,'') = ? AND COALESCE(site,'') = ?
     ORDER BY stat_date`;
  const trendRows = await optionalAllLocal(db, sql, params);
  const snapshotRows = await optionalAllLocal(db, snapshotSql, params);
...
module.exports = {
  ...
  _internal: {
    ...
    syncSiteExceptionsFromCapture, buildSiteExceptionList
  }
};