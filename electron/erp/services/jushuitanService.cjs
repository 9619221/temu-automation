...
class JushuitanService {
  ...
  ensureDefaultSources(companyId) {
    ...
    const run = this.db.transaction(() => {
      for (const source of SOURCE_CATALOG) insert.run(sourceToRow(source, companyId));
    });
    run();
  }

  ensureImportedSource(companyId, sourceKey, meta = {}) {
    this.ensureDefaultSources(companyId);
    const existing = this.db.prepare(`
      SELECT * FROM erp_jst_sync_sources WHERE company_id = ? AND source_key = ?
    `).get(companyId, sourceKey);
    ...
    const row = {
      ...
      method: optionalString(meta.method) || `file.${sourceKey}`,
      label: optionalString(meta.label) || "未识别导入表",
      category: optionalString(meta.category) || "导入",
      enabled: 1,
      sync_mode: normalizeSyncMode(meta.syncMode || meta.sync_mode || "static"),
      page_size: Number(meta.pageSize || meta.page_size) || 500,
      default_params_json: JSON.stringify(meta.defaultParams || meta.default_params || {}),
      cursor_field: null,
      cursor_value: null,
      last_synced_at: null,
      last_success_at: null,
  ...
  getAuthStatus(actor = {}) {
    ...
    this.ensureDefaultSources(companyId);
    const auth = toAuthStatus(this.getAuthRow(companyId));
    const sources = this.listSources({ companyId }, actor);
    const latestJobs = this.listJobs({ companyId, limit: 5 }, actor);
    const rawCount = this.db.prepare("SELECT COUNT(*) AS count FROM erp_jst_raw_records WHERE company_id = ?").get(companyId)?.count || 0;
    return {
      companyId,
      auth,
  ...
  saveAuthConfig(payload = {}, actor = {}) {
    ...
    const baseUrl = optionalString(payload.baseUrl || payload.base_url)
      || (environment === "sandbox" ? DEFAULT_SANDBOX_BASE_URL : DEFAULT_LEGACY_BASE_URL);
    const existing = this.getAuthRow(companyId);
    const row = {
      id: existing?.id || createId("jst_auth"),
      company_id: companyId,
      label: optionalString(payload.label) || existing?.label || "default",
      auth_mode: "legacy",
      environment,
      base_url: baseUrl,
      partner_id: optionalString(payload.partnerId || payload.partner_id) || existing?.partner_id || null,
      partner_key: optionalString(payload.partnerKey || payload.partner_key) || existing?.partner_key || null,
      token: optionalString(payload.token) || existing?.token || null,
      app_key: optionalString(payload.appKey || payload.app_key) || existing?.app_key || null,
      app_secret: optionalString(payload.appSecret || payload.app_secret) || existing?.app_secret || null,
      access_token: optionalString(payload.accessToken || payload.access_token) || existing?.access_token || null,
      status: normalizeStatus(payload.status || existing?.status),
      last_test_at: existing?.last_test_at || null,
      last_token_refresh_at: existing?.last_token_refresh_at || null,
      last_error: null,
      created_by: existing?.created_by || optionalString(actor.id) || null,
    ...
    `).run(row);
    this.ensureDefaultSources(companyId);
    return toAuthStatus(this.getAuthRow(companyId));
  }

  createClient(companyId, options = {}) {
    const row = this.getAuthRow(companyId);
    if (!row || !row.partner_id || !row.partner_key || !row.token) {
      throw new Error("请先配置聚水潭 partnerId / partnerKey / token");
    }
  ...
  async testConnection(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const client = this.createClient(companyId, { timeoutMs: Number(payload.timeoutMs) || 20000 });
    const now = nowIso();
    try {
      const response = await client.request("shops.query", {}, { timeoutMs: Number(payload.timeoutMs) || 20000 });
      this.db.prepare(`
        UPDATE erp_jst_auth_settings
        SET last_test_at = @last_test_at, last_error = NULL, updated_at = @updated_at
        WHERE company_id = @company_id
      `).run({ company_id: companyId, last_test_at: now, updated_at: now });
      const records = extractRecords(response);
      return { ok: true, method: "shops.query", recordCount: records.length, responseSummary: summarizeResponse(response, records, 1) };
    } catch (error) {
      this.db.prepare(`
        UPDATE erp_jst_auth_settings
  ...
  listSources(params = {}, actor = {}) {
    ...
    return rows.map((row) => ({ ...toSource(row), rawCount: row.raw_count || 0 }));
  }

  saveSource(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const sourceKey = optionalString(payload.sourceKey || payload.source_key);
    if (!sourceKey) throw new Error("sourceKey is required");
    this.ensureDefaultSources(companyId);
    const existing = this.db.prepare(`
      SELECT * FROM erp_jst_sync_sources WHERE company_id = ? AND source_key = ?
    `).get(companyId, sourceKey);
    if (!existing) throw new Error(`Jushuitan source not found: ${sourceKey}`);
    const defaultParams = payload.defaultParams && typeof payload.defaultParams === "object"
      ? payload.defaultParams
      : parseJsonObject(payload.defaultParamsJson || payload.default_params_json, parseJsonObject(existing.default_params_json, {}));
    const row = {
      company_id: companyId,
      source_key: sourceKey,
      enabled: payload.enabled === undefined ? existing.enabled : (payload.enabled ? 1 : 0),
      page_size: Math.max(1, Math.min(Math.floor(Number(payload.pageSize || payload.page_size || existing.page_size || 100)), 500)),
      default_params_json: JSON.stringify(defaultParams || {}),
      updated_at: nowIso(),
    };
    this.db.prepare(`
    ...
    `).run(row);
    return toSource(this.db.prepare("SELECT * FROM erp_jst_sync_sources WHERE company_id = ? AND source_key = ?").get(companyId, sourceKey));
  }

  createJob({ companyId, source, payload, actor }) {
    const now = nowIso();
    const row = {
      id: createId("jst_job"),
      company_id: companyId,
      ...
      mode: payload?.mode || (payload?.full ? "full" : "incremental"),
      status: "running",
      started_at: now,
      finished_at: null,
      fetched_count: 0,
      ...
      created_by: optionalString(actor?.id) || null,
    };
    this.db.prepare(`
  ...
  readWorkbookSheets(filePath) {
    const ext = path.extname(filePath).toLowerCase();
    if (![".xlsx", ".xls", ".csv"].includes(ext)) {
      throw new Error("只支持导入 .xlsx / .xls / .csv 文件");
    }
    ...
    return workbook.SheetNames.map((sheetName) => {
      ...
      const rows = XLSX.utils.sheet_to_json(sheet, {
        ...
        raw: false,
        blankrows: false,
      }).map(sanitizeSheetRow).filter((row) => !isBlankRow(row));
      const headers = rows[0] ? Object.keys(rows[0]) : [];
      return { sheetName, rows, headers };
    }).filter((sheet) => sheet.rows.length > 0);
  }

  importFile(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const filePath = optionalString(payload.filePath || payload.file_path);
    if (!filePath) throw new Error("filePath is required");
    const forcedSourceKey = optionalString(payload.sourceKey || payload.source_key);
    const sheets = this.readWorkbookSheets(filePath);
    if (!sheets.length) throw new Error("文件里没有可导入的数据行");

    const catalog = sourceCatalogByKey();
    const results = [];
    for (const sheet of sheets) {
      const inferredKey = forcedSourceKey || inferSourceKey({
        filePath,
        sheetName: sheet.sheetName,
        headers: sheet.headers,
      });
      const sourceMeta = catalog.get(inferredKey) || {
        key: inferredKey,
        method: `file.${inferredKey}`,
        label: inferredKey === "unknown" ? "未识别导入表" : inferredKey,
        category: "导入",
      };
      const source = this.ensureImportedSource(companyId, inferredKey, sourceMeta);
      ...
      try {
        const records = sheet.rows.map((row, index) => ({
          ...row,
          __jst_import_file: path.basename(filePath),
          __jst_import_sheet: sheet.sheetName,
          __jst_import_row: index + 2,
          __jst_imported_at: importedAt,
        }));
        this.storeRecords({ companyId, source, records, jobId: job.id, fetchedAt: importedAt });
        this.db.prepare(`
          UPDATE erp_jst_sync_sources
    ...
    return {
      ...
      importedRows: results.reduce((sum, item) => sum + Number(item.importedRows || 0), 0),
      results,
      importedAt: nowIso(),
    };
  }

  async openWebCollector(payload = {}, actor = {}) {
    normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const url = optionalString(payload.url) || DEFAULT_JST_URL;
    return this.getWebCollector().open({ url });
  }

  async closeWebCollector() {
    if (!this.webCollector) return { closed: true };
    return this.webCollector.close();
  }

  syncToOperationalTables(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const bridge = new JushuitanOperationalBridge({ db: this.db });
    return bridge.sync({ ...payload, companyId }, actor);
  }

  async collectWebPage(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const catalog = sourceCatalogByKey();
    const collectedAt = nowIso();
    const webResult = await this.getWebCollector().collect({
      url: optionalString(payload.url),
      maxPages: payload.maxPages || payload.max_pages,
      maxScrolls: payload.maxScrolls || payload.max_scrolls,
      maxRecords: payload.maxRecords || payload.max_records,
      pageSize: payload.pageSize || payload.page_size,
      autoNext: payload.autoNext === true || payload.auto_next === true,
      captureNetwork: payload.captureNetwork !== false && payload.capture_network !== false,
    });

    const firstRecord = webResult.records?.[0] || {};
    const inferredKey = optionalString(payload.sourceKey || payload.source_key)
      || inferSourceKey({
        filePath: webResult.url,
        sheetName: [webResult.title, firstRecord.__jst_web_page_heading, firstRecord.__jst_web_json_path].filter(Boolean).join(" "),
        headers: Object.keys(firstRecord),
      });
    const sourceKey = inferredKey && inferredKey !== "unknown" ? inferredKey : "web_collect";
    const sourceMeta = catalog.get(sourceKey) || {
      key: sourceKey,
      method: `web.${sourceKey}`,
      label: sourceKey === "web_collect" ? "网页采集" : sourceKey,
      category: "网页",
      syncMode: "static",
      pageSize: 500,
    };
    const source = this.ensureImportedSource(companyId, sourceKey, sourceMeta);
    const job = this.createJob({
      companyId,
      source,
      payload: {
        ...
        sourceKey,
        maxPages: payload.maxPages || payload.max_pages || 1,
        autoNext: payload.autoNext === true || payload.auto_next === true,
      },
      actor,
    });
    ...
    try {
      const records = mergeRecordsByExternalId((webResult.records || []).map((record, index) => ({
        ...record,
        __jst_web_source: "browser",
        __jst_web_record_index: index + 1,
        __jst_web_collected_at: collectedAt,
      })), sourceKey);
      if (records.length > 0) {
        this.storeRecords({ companyId, source, records, jobId: job.id, fetchedAt: collectedAt });
      }
      this.db.prepare(`
        UPDATE erp_jst_sync_sources
        SET last_synced_at = @last_synced_at,
      ...
      this.updateJob(job.id, {
        ...
        fetched_count: records.length,
        page_count: webResult.pageCount || 1,
        error: null,
        response_summary_json: {
          importType: "web",
          url: webResult.url,
          title: webResult.title,
          sourceKey,
          domCount: webResult.domCount,
          networkCount: webResult.networkCount,
          loginLikely: webResult.loginLikely,
        },
      });
      return {
        ...webResult,
      ...
      this.updateJob(job.id, {
        ...
        finished_at: nowIso(),
        fetched_count: 0,
        page_count: webResult.pageCount || 0,
        error: message,
        response_summary_json: {
          importType: "web",
          url: webResult.url,
  ...
  ingestExtensionBatch(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    ...
    for (const item of items) {
      if (!item || typeof item !== "object" || !isJushuitanCaptureItem(item)) {
        ignored += 1;
        continue;
      }
      const body = parseExtensionBody(item);
      if (!body) {
        ignored += 1;
        continue;
      }
      const records = [];
      collectExtensionJsonRecords(body, records);
      if (!records.length) {
        ignored += 1;
        continue;
      }
      const sourceKey = optionalString(item.sourceKey || item.source_key || body.sourceKey || body.source_key)
        || inferExtensionSourceKey(item, records[0]);
      if (!groups.has(sourceKey)) groups.set(sourceKey, []);
      const bucket = groups.get(sourceKey);
      records.forEach((record, index) => {
        bucket.push({
          ...record,
          __jst_web_source: "extension",
          __jst_web_record_index: bucket.length + index + 1,
          __jst_web_collected_at: collectedAt,
          __jst_web_response_url: item.url || item.tab_url || "",
          __jst_web_response_status: item.status || null,
          __jst_web_page: item.page || "",
          __jst_web_tab_url: item.tab_url || "",
        });
      });
    ...
    const sources = [];
    let importedRows = 0;
    for (const [sourceKey, rawRecords] of groups.entries()) {
      const sourceMeta = catalog.get(sourceKey) || {
        key: sourceKey,
        method: `extension.${sourceKey}`,
        label: sourceKey === "web_collect" ? "extension_capture" : sourceKey,
        category: "extension",
        syncMode: "static",
        pageSize: 500,
      };
      const source = this.ensureImportedSource(companyId, sourceKey, sourceMeta);
      const records = mergeRecordsByExternalId(rawRecords, sourceKey);
      const job = this.createJob({
        companyId,
        source,
        payload: {
          mode: "extension_ingest",
          sourceKey,
          eventCount: items.length,
          extractedRows: rawRecords.length,
        },
        actor,
      });
      try {
        ...
        importedRows += records.length;
        sources.push({ sourceKey, extractedRows: rawRecords.length, importedRows: records.length, jobId: job.id });
      } catch (error) {
        this.updateJob(job.id, {
          status: "failed",
          finished_at: nowIso(),
          ...
          response_summary_json: { importType: "extension", sourceKey },
        });
        throw error;
      }
    }

    const sourceKeys = sources.map((item) => item.sourceKey);
    const syncResult = importedRows > 0 && payload.sync !== false
      ? this.syncToOperationalTables({ companyId, sourceKeys }, actor)
      : null;
    return {
      ok: true,
      companyId,
      acceptedEvents: items.length - ignored,
  ...
  storeRecords({ companyId, source, records, jobId, fetchedAt }) {
    ...
    const run = this.db.transaction(() => {
      for (const record of records) {
        const rawJson = stableStringify(record);
        ...
        stmt.run({
          ...
          method: source.method,
          external_id: inferExternalId(record, source.source_key),
          cursor_value: inferCursorValue(record) || null,
  ...
  updateJob(jobId, patch) {
    const current = this.db.prepare("SELECT * FROM erp_jst_sync_jobs WHERE id = ?").get(jobId);
    if (!current) return null;
    const row = {
      ...current,
      ...patch,
      response_summary_json: patch.response_summary_json !== undefined
        ? (typeof patch.response_summary_json === "string" ? patch.response_summary_json : JSON.stringify(patch.response_summary_json || {}))
        : current.response_summary_json,
    };
    this.db.prepare(`
  ...
  async syncSource(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    const sourceKey = optionalString(payload.sourceKey || payload.source_key);
    if (!sourceKey) throw new Error("sourceKey is required");
    this.ensureDefaultSources(companyId);
    const source = this.db.prepare(`
      SELECT * FROM erp_jst_sync_sources WHERE company_id = ? AND source_key = ?
    `).get(companyId, sourceKey);
    if (!source) throw new Error(`Jushuitan source not found: ${sourceKey}`);
    if (!source.enabled && !payload.force) throw new Error(`Jushuitan source is disabled: ${sourceKey}`);

    const client = this.createClient(companyId, { timeoutMs: Number(payload.timeoutMs) || 30000 });
    const job = this.createJob({ companyId, source, payload: { ...payload, mode: payload.mode || "single" }, actor });
    const maxPages = source.sync_mode === "static" ? 1 : Math.max(1, Math.min(Math.floor(Number(payload.maxPages) || 200), 10000));
    const summaries = [];
    let fetchedCount = 0;
    let pageCount = 0;
    let lastCursor = optionalString(source.cursor_value);
    try {
      for (let pageIndex = 1; pageIndex <= maxPages; pageIndex += 1) {
        const { body, pageSize } = buildRequestBody(source, {
          ...payload,
          pageIndex,
          pageSize: payload.pageSize || payload.page_size || source.page_size,
        });
        const response = await client.request(source.method, body, { timeoutMs: Number(payload.timeoutMs) || 30000 });
        const records = extractRecords(response);
        const fetchedAt = nowIso();
        if (records.length) {
          this.storeRecords({ companyId, source, records, jobId: job.id, fetchedAt });
          fetchedCount += records.length;
          for (const record of records) {
            const cursor = inferCursorValue(record);
            if (cursor && cursor > lastCursor) lastCursor = cursor;
          }
        }
        pageCount = pageIndex;
        summaries.push(summarizeResponse(response, records, pageIndex));
        if (source.sync_mode === "static" || !responseHasMore(response, pageIndex, pageSize, records.length)) break;
      }
      const now = nowIso();
      this.db.prepare(`
        UPDATE erp_jst_sync_sources
      ...
      this.updateJob(job.id, {
        ...
        error: null,
        response_summary_json: { pages: summaries },
      });
      return {
        jobId: job.id,
        sourceKey,
  ...
  async syncAll(payload = {}, actor = {}) {
    const companyId = normalizeCompanyId(payload.companyId || payload.company_id, actor);
    this.ensureDefaultSources(companyId);
    const requested = Array.isArray(payload.sourceKeys || payload.source_keys)
      ? (payload.sourceKeys || payload.source_keys).map(optionalString).filter(Boolean)
      : [];
    const allSources = this.listSources({ companyId }, actor)
      .filter((source) => source.enabled && (!requested.length || requested.includes(source.sourceKey)));
    const results = [];
  ...
  getBusinessSummary(params = {}, actor = {}) {
    const companyId = normalizeCompanyId(params.companyId || params.company_id, actor);
    const rawCount = this.db.prepare("SELECT COUNT(*) AS count FROM erp_jst_raw_records WHERE company_id = ?").get(companyId)?.count || 0;
    const businessCount = this.db.prepare("SELECT COUNT(*) AS count FROM erp_jst_business_records WHERE company_id = ?").get(companyId)?.count || 0;
    ...
    return {
      ...
      rawCount,
      businessCount,
      accounts: this.db.prepare("SELECT COUNT(*) AS count FROM erp_accounts WHERE company_id = ? AND id LIKE 'jst:%'").get(companyId)?.count || 0,
      suppliers: this.db.prepare("SELECT COUNT(*) AS count FROM erp_suppliers WHERE company_id = ? AND id LIKE 'jst:%'").get(companyId)?.count || 0,
      skus: this.db.prepare("SELECT COUNT(*) AS count FROM erp_skus WHERE company_id = ? AND id LIKE 'jst:%'").get(companyId)?.count || 0,
      purchaseOrders: this.db.prepare("SELECT COUNT(*) AS count FROM erp_purchase_orders WHERE account_id = 'jst:account:default'").get()?.count || 0,
      sku1688Sources: this.db.prepare("SELECT COUNT(*) AS count FROM erp_sku_1688_sources WHERE id LIKE 'jst:%'").get()?.count || 0,
      bySource,
  ...
  listBusinessRecords(params = {}, actor = {}) {
    const companyId = normalizeCompanyId(params.companyId || params.company_id, actor);
    const sourceKey = optionalString(params.sourceKey || params.source_key);
