 ...
 * 「预览」= getAutoPurchaseCandidates(只读)；「申请」= applyAutoPurchaseBatch(真下单，前端二次确认后调)。
 */
...
function buildPendingQtyMap(db, nowMs) {
  const map = new Map();
 ...
 */
function getAutoPurchaseCandidates(db, opts = {}) {
  const mallId = opts.mallId ? String(opts.mallId) : null;
  const nowMs = Date.now();
  const pendingMap = buildPendingQtyMap(db, nowMs);
  const costMap = buildCostMap(db);

  const hour = beijingHour();
  // 扫全量 SKU，用「自算建议备货」（销量+总库存）替代 Temu 黑盒 advice_qty。
  const rows = mallId
    ? db.prepare("SELECT * FROM erp_temu_openapi_sku_sales WHERE mall_id = ?").all(mallId)
    : db.prepare("SELECT * FROM erp_temu_openapi_sku_sales").all();
