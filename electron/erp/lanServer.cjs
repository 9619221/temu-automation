...
cloud_only AS (
  SELECT
    c.cloud_so AS so_id,
    'cloud' AS source,
    CAST(NULL AS INTEGER) AS jst_o_id,
    NULL AS jst_shop_name,
    NULL AS jst_status,
    NULL AS jst_src_status,
    NULL AS jst_shop_status_text,
    CAST(NULL AS NUMERIC) AS jst_item_amount,
    CAST(NULL AS INTEGER) AS jst_items_qty,
    ...
    CAST(NULL AS BIGINT) AS jst_local_ship_total,
    c.cloud_so,
    c.cloud_row_key,
    c.cloud_mall_id,
    c.cloud_site,
...
async function enrichSendAddresses(db, payloadRows) {
  ...
  try {
    ...
    for (const row of await queryAll(db, "SELECT mall_id, send_address_json FROM erp_temu_malls WHERE mall_id IN (" + [...mallIds].map(() => "?").join(",") + ") AND send_address_json IS NOT NULL", [[...mallIds]])) {
...
async function _getSnapColInfo(db) {
  ...
  if (!hasOnlineStatus) {
    try {
      ...
      await execSql(db, "CREATE INDEX IF NOT EXISTS idx_consign_unified_snap_company_online_status ON temu_consign_unified_snapshot(company_id, online_status)");
      await execSql(db, `UPDATE temu_consign_unified_snapshot SET online_status = json_extract(payload_json, '$.rawCloud.temu_status') WHERE online_status IS NULL`);
  ...
  if (!hasShopName) {
    try {
      ...
      await execSql(db, "CREATE INDEX IF NOT EXISTS idx_consign_unified_snap_company_shop ON temu_consign_unified_snapshot(company_id, shop_name)");
      await execSql(db, `UPDATE temu_consign_unified_snapshot SET shop_name = json_extract(payload_json, '$.shopName') WHERE shop_name IS NULL`);
      hasShopName = true;
...
async function readConsignDeliveriesUnifiedFromSnapshot(db, opts) {
  ...
  "SELECT rebuilt_at AS m FROM temu_consign_unified_snapshot WHERE company_id = ? LIMIT 1", [
  companyId]);
  if (!meta) return null;
  // 只要有数据就用，不检查陈旧度（统一表由写入端实时维护）

  const { statusCol, hasOnlineStatus, hasShopName } = await _getSnapColInfo(db);
  ...
  const hasSourceFilter = source === "cloud" || source === "jst" || source === "both";const
  ...
  { rows, aggRow, totalCount, statusBreakdown, onlineStatusBreakdown } = await withTransaction(db, async (txDb) => {const rowsQ = buildWhere(true);const rows = await queryAll(txDb, `
      SELECT payload_json FROM temu_consign_unified_snapshot
      ${rowsQ.where}
      ORDER BY order_key DESC, so_id DESC
      LIMIT @limit OFFSET @offset
    `, { ...rowsQ.values, limit: pageSize, offset });const bdQ = buildWhere(false);const aggRow = await queryOne(txDb, `
      SELECT
        COUNT(*) AS total,
    ...
    `, [bdQ.values]);const totalCount = hasSourceFilter ? Number(aggRow?.[`src_${source}`] || 0) : Number(aggRow?.total || 0);const sbKey = `${companyId}\0${search || ""}`;const sbCached = _sbBreakdownCache.get(sbKey);let statusBreakdown, onlineStatusBreakdown;if (sbCached && Date.now() - sbCached.t < 300000) {statusBreakdown = sbCached.s;onlineStatusBreakdown = sbCached.o;} else {const sbValues = { company_id: companyId };const sbCond = ["company_id = @company_id"];if (search) {sbValues.search = `%${search}%`;sbCond.push("search_blob LIKE @search");}const combined = await queryAll(txDb, `
      ...
      `, [sbValues]);statusBreakdown = {};onlineStatusBreakdown = {};for (const r of combined) {const sKey = r.status == null || r.status === "" ? "(空)" : String(r.status);statusBreakdown[sKey] = (statusBreakdown[sKey] || 0) + Number(r.n || 0);if (hasOnlineStatus && r.online_status) {const oKey = String(r.online_status);onlineStatusBreakdown[oKey] = (onlineStatusBreakdown[oKey] || 0) + Number(r.n || 0);}}_sbBreakdownCache.set(sbKey, { s: statusBreakdown, o: onlineStatusBreakdown, t: Date.now() });}return { rows, aggRow, totalCount, statusBreakdown, onlineStatusBreakdown };});const sourceBreakdown = { cloud_only: Number(aggRow?.src_cloud || 0), jst_only: Number(aggRow?.src_jst || 0), both: Number(aggRow?.src_both || 0) };const payloadRows = rows.map((r) => JSON.parse(r.payload_json));await enrichSendAddresses(db, payloadRows);return { ok: true, rows: payloadRows, total: totalCount, page, pageSize, sourceBreakdown, statusBreakdown, onlineStatusBreakdown, fromSnapshot: true };}

async function runConsignDeliveriesUnified(db, params = {}) {
  if (!db) {
    return {
      ok: true,
      rows: [],
...
async function runConsignDeliveriesUnifiedJstOnly(db, opts) {
  ...
  const totalRow = await queryOne(db, `SELECT COUNT(*) AS n FROM jst_consign_deliveries ${whereClause}`, [values]);
...
async function readOptionalPayload(req, maxBytes = 16 * 1024) {
  ...
  return await readLoginPayload(req, maxBytes);
}
...
async function handleRequest({
  ...
  if (protectedPath && session?.user) {
    try {
      const scope = await resolveStoreScope(session.user);
  ...
  if (pathname === "/api/master-data/supplier-goods") {
    ...
    const rows = await queryAll(db, `
        ...
        LIMIT @limit
      `, { supplier_id: supplierId || undefined, company_id: companyId, limit });
  ...
  if (pathname === "/api/master-data/consign-deliver-cloud-items") {
    ...
    let rows = [];
    if (mallId && soId) {
      const row = await queryOne(db, "SELECT items_json FROM erp_temu_openapi_consign WHERE mall_id = ? AND so_id = ?", [String(mallId), String(soId)]);
      ...
      if (rows.length) {
        try {
          const st = await queryOne(db, "SELECT ship_qty_json FROM erp_consign_local_state WHERE mall_id = ? AND so_id = ?", [String(mallId), String(soId)]);
          let shipMap = {};
  ...
  if (pathname === "/api/erp/reports/yunqi-selection-ids") {
    try {const accountId = new URL(req.url || "/", "http://127.0.0.1").searchParams.get("accountId") || "";writeJson(res, 200, { ok: true, data: await require("./services/yunqiCloud.cjs").listSelectionIds(accountId) });}
    catch (error) {writeJson(res, 500, { ok: false, error: error?.message || String(error) });}
    return;
  }
  ...
  if (pathname === "/api/erp/reports/yunqi-selection-remove") {
    if (req.method !== "POST") {writeJson(res, 405, { ok: false, error: "Method not allowed" });return;}
    try {const p = await readOptionalPayload(req);writeJson(res, 200, { ok: true, data: await require("./services/yunqiCloud.cjs").removeSelection(p?.goodsId, p?.accountId) });}
    catch (error) {writeJson(res, error?.statusCode || 500, { ok: false, error: error?.message || String(error) });}
    return;
  }
  if (pathname === "/api/erp/reports/yunqi-selection-update") {
    if (req.method !== "POST") {writeJson(res, 405, { ok: false, error: "Method not allowed" });return;}
    try {const p = await readOptionalPayload(req);writeJson(res, 200, { ok: true, data: await require("./services/yunqiCloud.cjs").updateSelection(p?.goodsId, { status: p?.status, note: p?.note, accountId: p?.accountId }) });}
    catch (error) {writeJson(res, error?.statusCode || 500, { ok: false, error: error?.message || String(error) });}
    return;
  }
  ...
  if (pathname === "/api/erp/reports/yunqi-categories") {
    try {writeJson(res, 200, { ok: true, data: await require("./services/yunqiCloud.cjs").listCategories() });}
    catch (error) {writeJson(res, 500, { ok: false, error: error?.message || String(error) });}
    return;
  }
